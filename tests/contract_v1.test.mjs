...
// ⚠️ **Z1/Z2/Z3 曾带 `{ todo: … }` 标记**（2026-09-18 用户拍板 B：红态单独推上去会掩盖其他失败）。
//   **R9-1 实现落地后已摘除**（`src/convert.js` 读文件/嗅探兜底 + `src/app.js` 逐文件隔离与状态计数）
//   ⇒ 三条恢复为**真阻断**。今后凡"先红用例要单独落地"，沿用「标 todo → 实现时摘除」这一对动作。
// ---------------------------------------------------------------------------
  ...
  try {
    ...
    try {
      ...
      await t.test('G9-3 类型无关性：布尔与文本不动，同一张表里的数值照常归一', () => {
        ...
        // ⚠️ 本断言**同时覆盖两种失败模式**（非数值被误改 / 数值未归一），故报错文案必须两种都点到 ——
        //    首版只写「非数值被误改」，在"关掉归一"的注入下会指向错误的方向（Linux 侧验收登记，2026-09-20）。
        //    **只改文案，断言条件一字未动。**
        assert.equal(
          md,
          '### Sheet: S1\n\n' + tableOf(['true', '文本 1.2345678901234567', '34.2']),
          `本格应同时满足「布尔/文本格原样」与「数值格归一为 34.2」两者之一被破坏即红（按 actual/expected 逐格判读）：${JSON.stringify(md)}`
        );
      });
