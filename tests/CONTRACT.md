...
| C3 | 转换耗时（外部计时，convert 调用起止） | `< 500ms` | 🔴 红（口径见拍板点 T-1；**t23 补充**：image 用例按 T-1 冷启动豁免设 per-case `thresholdMs: 5000` 豁免窗口（lazy-init 本地模型加载无法用外部计时分离；主口径 <500ms 保留——预热后/二次 OCR 计时））· **2026-09-19 改按 T-1「B′」执行（原 5000ms 豁免窗口作废，保留作历史）**：image 用例**同页跑两次** —— 第 1 次冷启动**只判「没卡死」**（防呆 `coldCeilingMs: 20000`，**非性能判据**），第 2 次（**预热后**）按**主口径 `<500ms`** 断言。依据 = 2026-09-18 实测该窗口偶发超时（10,194 ms；Linux 侧控制变量：基线全绿 / 补丁 B1 红 B2 绿 ⇒ flaky，与补丁无关） |
...
来源：**用户 2026-09-20 实测**（观察期「去用」）—— 原话「我自己试了一下，我看这**括号**有问题啊」。现象 = alt 里的空行让 `![alt](src)` **跨行** ⇒ 渲染器断在空行处 ⇒ 页面上只剩**裸的 `![` / `](`**。

根因（**两条入口，不许只测一条**）：① `src/docx.js` `docxAltFromName` 只有 `.trim()`（只削首尾），内部 `\n\n` 原样进 `![alt](src)`；② `src/html2md.js` `imgFrag` 只把 `]` 转义成 `%5D`，**同样没处理空白** —— 而 docx 路径的 alt **最终也经它**产出（mammoth → HTML `<img alt>` → `htmlToMarkdown`）⇒ 两条入口是同一条链。

口径（用户 2026-09-20 拍板，写死）：连续空白（含 `\n` / `\r\n` / `\t` / 多空格）**压成单个空格** —— ① **不删除**（保住无障碍信息）② **不留多个** ③ 产物里 `![alt](src)` **必须单行**。⚠️ `html2md` 侧**只折叠、不裁剪首尾**（裁剪会动到"无换行输入"的字节；docx 侧的首尾 trim 是它本来就有的口径）。

样例：**无新增入库样例** —— 用例**页内现造**（`window.fflate.zipSync`，与 G7/G8/S3 同构；`pic:cNvPr` 的 `descr`/`name` 用字符引用 `&#10;` / `&#9;` 写真空白 —— ⚠️ **字面换行会被 XML 属性值归一化成空格**，写错就造不出红回路）。**既有夹具字节锁零改动**（`manifest.json` 未动）。

| 编号 | 断言 | 标准 | 当前 |
|---|---|---|---|
| I2-1 | docx `descr` 含空行 ⇒ 产物单行且**空行压成恰好一个空格** | 逐字符相等 | ✅ 绿（**先红实测**：`![文本, 信件\n\nAI 生成的内容可能不正确。](…)`） |
| I2-2 | docx `name` 回落路径同样折叠（去扩展名口径不变） | 逐字符相等 | ✅ 绿（先红实测：`![图片\n\n2057007599](…)`） |
| I2-3 | docx 制表符 / 连续空格 / 换行 ⇒ 各自压成一个空格 | 逐字符相等 | ✅ 绿（先红实测：`![A\tB   C\nD](…)`） |
| I2-4 | html2md：`\n\n` / CRLF / `\t` / 双空格全部折叠；**`]`→`%5D` 既有口径保留** | 逐字符相等 ×5 | ✅ 绿（先红实测：`![a\n\nb](x.png)`） |
| I2-5 | **A3 无障碍信息保住**：清洗前后「去空白」逐字符相等 + alt 内无 `[\r\n\t]` 与连续空格 | 相等 + 正则 | ✅ 绿 |
| I2-6 | **A4 回归护栏（控制组）**：无连续空白者**一字不变**（docx + html2md 各一例） | 逐字符相等 | 🟢 绿（先红阶段即绿 —— 防误伤 C1 口径） |

**两相实测（本机 · 卡 006）**：**先红** = `node --test --test-name-pattern="契约组 I2"` → `tests 7 / pass 1 / fail 6`（exit 1）；**后绿** = 同命令 `7/7`、全量 **290 / 288 / 0 / 2**。

⚠️ **端到端复验（A10）**：等价构造的「15 张图 + 每张 alt 含空行」样例 ⇒ 产物 **15/15 单行**，`markdown md2html` 渲染 **15/15 `<img>`、页面零裸 `![`**（改前 **0/6**）。用户原件（论文 `实验.docx`）**不入库**。
...
### 契约组 G9 — xlsx 数值长尾的**显示归一**（渲染层；卡 007；2026-09-20 契约先红）

来源：卡 002 回执自登记的第 ② 条待拍板（手机 4.2 实测「**50 行**由 `34.2` 变 `34.200000000000003`」）⇒ **用户 2026-09-20 拍板「没做先做」**，并给定权威口径：**在渲染层格式化（非解析层）**。

根因（**实测**）：夹具 XML 里存的就是长形态，我方快路径**忠实读出**（`serialDateOrRaw` 直接返回字面量）⇒ 可读性回退；**旧库路径做过数值归一**（`read-excel-file` 转 JS number，`String(number)` 即短形态）。

⭐ **两道开工前必答题的实测答案**（卡面要求"先测再判"）：
1. **四档夹具里有没有浮点长尾？** ⇒ **有**（窗口 = 输出的前 1000 行）：`1-small` **266** 个 · `2-mid` **0** 个 · `4-mid-large` **3,039** 个 · `3-big` **3,039** 个 ⇒ **A5 走「新值 + 逐档说明」分支**（见 §2 契约组 Y 的「口径变更登记」）。
2. **「50 行」是什么？** ⇒ **`4-mid-large`/`3-big` 的 R 列恰好 50 个长尾单元格**（行 38, 98–101, 187, … 975；首例 **`R38 = 34.200000000000003`**）—— 与手机侧「50 行」**逐字对上**。⚠️ 同两档 D/J/K 列另有 ≈2,989 个长尾，但它们是 **17 位日期序列号 + 日期样式** ⇒ 走日期路径、**不受本规则影响**。

**口径（写死，断言即规格）**：

| 输入 | 判定 | 输出 |
|---|---|---|
| 十进制**有效数字 > 15 位** 且**含小数点** | 表示噪声 | **15 位有效数字**形态：`String(Number(Number(v).toPrecision(15)))` |
| 有效数字 ≤ 15 位 | 真精度/常态 | **原样**（含 `1.0` / `0.0000001` / `1e-7` / 14 位真精度） |
| **整数**（无小数点） | —— | **一律原样**（20 位整数也不动 —— >2^53 改写即撒谎） |
| 日期样式命中 | —— | 走 `excelSerialToDate`（**本规则不介入**） |

⚠️ **为什么用「15 位显示」而不是更窄的「无损 round-trip」规则**：卡面 A4 明确要求 `0.30000000000000004 → 0.3`，而该字面量与 `0.3` **不是同一个 double** ⇒ 任何只做无损 round-trip 的规则都产不出 `0.3` ⇒ **规则被 A4 钉死**。代价（**已登记**）：16–17 位字面量的第 16 位起按显示舍入（如 `3.141592653589793 → 3.14159265358979`）—— 这正是 Excel 的 15 位精度显示模型。

| 编号 | 断言 | 标准 | 当前 |
|---|---|---|---|
| G9-1 | 长尾归一：`34.200000000000003→34.2` · `0.30000000000000004→0.3`（16 位 → 15 位、20 位整数不动、幂等） | 全表**逐字符**相等 | ✅ 绿（**先红实测**：原样吐出长尾） |
| G9-2 | **护栏**：有效数字 ≤15 位的 9 例**逐字符原样**（`1` / `1.5` / `0.001` / `482547666944` / `1000000` / `1.0` / `0.0000001` / `1e-7` / `1.2345678901234`） | 逐字符相等 | 🟢 绿（先红阶段即绿） |
| G9-3 | **类型无关性**：布尔与**长得像长尾的文本**不动，同表内数值照常归一 | 逐字符相等 | ✅ 绿（先红：数值那格未归一） |
| G9-4 | **日期样式走日期路径**：序列号单元格出 ISO 日期、**不含原序列号** | 正则 + 不含 `46282` | 🟢 绿（先红阶段即绿 —— 日期路径本来就对） |
| G9-5 | 同值但**无样式** ⇒ 属数值：`46282.61273148148 → 46282.6127314815` | 严格相等 | ✅ 绿（先红：原样） |
| G9-6 | `backend` / `meta.scan.rows` / `warnings` 与既有语义一致（渲染层生效但不改解析口径） | 严格相等 | 🟢 绿 |

**两相实测（本机 · 卡 007）**：**先红** = `node --test --test-name-pattern="契约组 G9"` → `tests 7 / pass 3 / fail 4`（exit 1；G9-1/G9-3/G9-5 红）；**后绿** = 同命令 **7/7**、全量 **297 / 295 / 0 / 2**（带夹具）/ **294 / 291 / 0 / 3**（干净检出）。

⭐ **A3（解析层不变）** 的硬证据 = **函数级逐字比对**（`git worktree` 取开工基线 `416e7d2`）：解析阶段 **10 个函数**（`findTagStartInfo` / `parseCellAt` / `parseRowCells` / `textPuller` / `takeRowUnit` / `scanSheetRowsStream` / `colIndexOfRef` / `rowToTexts` / `decodeXml` / `xlsxParseSheetStream`）+ 渲染阶段**另外 3 个**（`cellToString` / `xlsxRowsToMd` / `excelSerialToDate`）**全部 byte-identical**；变的只有 `serialDateOrRaw`（值→文本那一步）与新增的 `sigDigitsOf` / `formatNumericLiteral`。
...
口径（用户 2026-09-11 拍板口径 A **全篇判定**；**2026-09-15 收尾批把阈值 2/10 → 3/4**，见 §7 2026-09-15 条）：`decodeText` 的编码判定改为全篇——UTF-8 fatal 快检先行（合法即返回，零额外成本），非法才做全篇 U+FFFD 计数；**结构判据门 = `fffd ≥ 3 && nonAscii > 0 && fffd × 4 ≥ nonAscii`**（阈值来由 = 三套阈值 × 17 例实测矩阵）；「gb18030 侧 FFFD 严格更少才采用」的次级比较不变（F 组 F1–F7 语义不动）。实现 = `src/sniff.js` `decodeText`。
...
| S4-5 | 收口批（2026-09-11）· 大体合法 UTF-8 + 2 处多字节截断（13,010 B，页内现造） | 保持 UTF-8：含「你好世界」、无「浣犲ソ」、U+FFFD ≤4 | 🟢 绿（先红 → A′ 门后转绿；`6a891a2` 先红 / `217459f` 实现。**本行 2026-09-15 补登记**——原表漏记，属文档漂移） |
| S4-6 | 收口批 · 守护：8192 B ASCII 头 + 80,000 B 真 GBK 正文 | 仍回退 gb18030：含「中文测试」、无 U+FFFD | 🟢 绿（守护。**本行 2026-09-15 补登记**） |
| S4-7 | 收口批 · 守护 floor：纯 ASCII + 单字节损坏 0xB0 | 不触发回退：输出逐字符 = 6000×`A` + U+FFFD + `BCDEF` + 1000×`G` | 🟢 绿（守护。**本行 2026-09-15 补登记**） |
| S4-8 | 收尾批（2026-09-15）· **守卫**（重建 t12-C1）：纯 GBK 长文 80,000 B | 仍采纳 gb18030：含「中文测试」、无 U+FFFD | 🟢 绿（守卫 floor 不得误伤长 GBK） |
| S4-9 | 收尾批 · **守卫**（重建 t12-C2）：100,000 B ASCII + 200 B GBK 尾 | 仍采纳 gb18030：尾部中文正确、无 U+FFFD | 🟢 绿（守卫小 GBK 尾不被 floor 挡掉） |
| S4-10 | 收尾批 · **守卫**（重建 t12-C8）：S4-5 型 10,796 B + 2 处截断 | 保持 UTF-8：含「你好世界」、无「浣犲ソ」、U+FFFD ≤4 | 🟢 绿（守卫） |
| S4-11 | 收尾批 · **缺陷**（重建 t12-C4）：ASCII + 2×0xB0 | 保持 UTF-8：U+FFFD = 2、输出无汉字 | 🟢 绿（先红实测 U+FFFD=**0** —— ASCII 被 gb18030 整篇重解） |
| S4-12 | 收尾批 · **缺陷**（重建 t12-C10）：短 UTF-8 中文 + 2×0xB0（40 B） | 保持 UTF-8：含「你好世界」、无「浣犲ソ」 | 🟢 绿（先红实测整篇变 `浣犲ソ涓栫晫浣犲ソ涓栫晫浣犲ソ涓栫晫  鞍`） |
| S4-13 | 收尾批 · **缺陷**（重建 t12-C13）：ASCII + 2×0x80 | 保持 UTF-8：U+FFFD = 2、输出无 `€`（U+20AC） | 🟢 绿（先红实测 0x80 被解成 `€`） |
| S4-14 | 收尾批 · **边界登记**：2 字节单汉字 GBK（`中` = D6 D0） | floor=3 下**不再回退**：输出 = 2 个 U+FFFD、不含「中」 | 🟢 绿（**行为变更已登记**——与「2 处损坏」计数签名不可分，理由见 §7 2026-09-15 条） |

> **S4-8..S4-14 的样例形态（2026-09-15 收尾批）**：全部**页内现造、不入库样例**（与 S4-5/6/7 同规格，零仓库体积增长）。配方来自 AgentTeams `doc2md-s4s5` t12 交叉审查公布值，⚠️ **原始字节不可得**（生成脚本在 `.tmp/qa-t12/`，已被清理）→ **重建件**：判据值同量级、非同值（C1 原 fffd=52800/nonAscii=59200 → 重建 60000/70000；C8 的 nonAscii 原 3472 → 重建 1826），已在 `tests/contract_v1.test.mjs` 注释写明。
...
|---|---|---|---|
| U-0 | 7 个新增样例存在 + manifest 字节级一致（×7） | 大小/SHA 与 manifest 一致 | 🟢 绿（实测 7/7 一致；U9 样例 2026-09-16 补入） |
...
| U9 | **P1 缩放感知**（`sample-scale-td.pdf`，2026-09-16 补）：`Tm` 缩放 0.05 + **逐字 `TD` 推进**（真机 WPS 形态，同一视觉行两个文本对象）→ 行内顺序**不得交错** | **去空白后**：含 `ONETWO`（书写顺序拼接）、**不得**出现旧交错序 `OTNWEO`；且 `SECONDLINE` 在 `THIRDLINE` 之前 | 🟢 绿（**先红实测** `tests 11 / pass 9 / fail 2`：实际输出 `OSTNEHWECIOORNDDLLIINNEE`；实现 `9b6e9c7` 后 11/11） |
...
### 契约组 W — OCR 输入质量：预缩放 / 版面模式 / 质量信号（v0.1.6，2026-09-15 真机 4 张样张实测驱动；契约先红）

口径（用户 2026-09-15 拍板，逐条有实测支撑；证据链 `.私档/项目/真机OCR复现/grid-20260915/`）：

1. **大图预缩放**：长边 > 1500px 的图先等比缩到长边 1500px（**只缩不放**）。依据：12.6MP 真机照片实测 —— 原图 + 默认 PSM 6 探针 **2/5** → 50%/35% 缩放 + PSM 3 **5/5**，且耗时降 30–60%（6.7s → 4.1s）；更小的 ≈1024px 反而变差（12/16）→ 取 1434–2048 区间。
2. **显式设置 PSM 3（AUTO）**：tesseract.js 的**隐式默认是 6（SINGLE_BLOCK，不做版面分析）**，与 tesseract 命令行默认（3）不同。实测：默认 6 在「图旁正文」与「90° 旋转页」两例上分别**整段 / 整页崩**；PSM 4 在旋转页只剩 4–7 字符；PSM 3 在 4 张上**从不最差**，且是旋转页**唯一可读**配置（**旋转无需 OSD 词典资产**）。
3. **质量信号**：低置信度（既有，`conf < 60`）+ **高拉丁占比**（新增）——仅在含足量中文（CJK ≥ 20）且 `拉丁/(CJK+拉丁) > 0.4` 时提示「版面或方向问题」，以免把纯英文图误报。

实现 = `src/ocr.js`（`OCR_PSM` / `MAX_OCR_EDGE` / `prepareOcrImage`）+ `src/convert.js`（`imageConvert` 接入 + `ocrWarnings` 增判据）。

| 编号 | 断言 | 标准 | 当前（基线 `10930d9`，契约先红） |
|---|---|---|---|
| W1 | 行为级：worker 创建后须被**显式**设置 `tessedit_pageseg_mode=3`（假 worker 记录 `setParameters` 入参） | 至少一次 `setParameters` 且值为 3 | 🔴 红（先红实测 `params=[]`——产品从未设置过 PSM）→ 🟢 绿 |
| W2 | 行为级：3000×2000 输入 → 交给 `recognize` 的位图长边 ≤1500px 且宽高比保持 | 长边 ≤1500 且 宽/高 ≈1.5（±0.05） | 🔴 红（先红实测 `{w:3000,h:2000}`）→ 🟢 绿 |
| W3 | **守护**：800×600 输入不得放大（只缩不放） | 尺寸原样 800×600 | 🟢 绿（防过度修正） |
| W4 | 行为级：中文 43 字 + 拉丁乱码 59 字符 → warnings 须提示版面/方向 | 命中 `/(版面\|方向)/` | 🔴 红（先红实测 `warnings=[]`）→ 🟢 绿 |
| W5 | **守护**：正常中文 + 置信度 90 → 不得出现该提示 | 不命中 | 🟢 绿 |
| W6 | **守护**：纯英文图（`sample.png` 形态）+ 置信度 93 → 不得出现该提示 | 不命中 | 🟢 绿（拉丁占比必然高，但 CJK=0 → 不得判版面问题） |

**先红 → 后绿**：**先红** = 只跑组 W → `tests 7 / pass 3 / fail 4`（W1/W2/W4 + 组壳红；W3/W5/W6 按设计保持绿）；**后绿** = 全量 **240 tests / 238 pass / 0 fail / 2 skip**，产物 `119,699 B` / SHA `3D044AA0…4DB0`（blob `4adb0c82…`）。
**连带修正**：组 R 的 R3 假 worker 需补 `setParameters` 空实现（**测试桩随接口升级，R3 断言期望值一字未动**）。
**未进快契约（登记）**：真 OCR 的端到端**效果**不入快契约（保确定性与时长 ~51s）——本批口径由 4 张真机样张的**离线网格证据**支撑（证据链目录），契约组只锁「配置 + 行为契约」；端到端由 Linux 侧独立验收覆盖。

### 契约组 X — OCR 方向重试（v0.1.7，2026-09-15 产品路径实测驱动；契约先红）

**根因（实测）**：tesseract 的 AUTO 版面分析**只认一个旋转方向** —— 同一张接近真实比例的页面夹具（10 行中文，旋转后 1900×1400，与真机旋转照片同形）：顺时针转 90° → CJK **118/123**（可读）；**逆时针转 90° → CJK 7**（整篇乱码，conf 24）。产品路径（真 chromium + 真 OCR）复现一致，且对失败方向**再顺时针转 90°** 即读回（CJK 118）。

**口径（用户 2026-09-15 拍板）**：

1. **触发 = 与 `ocrWarnings` 完全同一口径**（会提示 → 就重试；"提示即行动"）⇒ **成本只落在坏例上**（好结果不重试）。
2. **动作 = 顺时针旋转 90° 重试一次**（`rotateImage90`）；**不引入 OSD 词典资产**（用户明确否决 +10MB 方案）。
3. **只重试一次**（无循环）/ **取更优的**（按写死打分 `conf − 100 × 拉丁占比`，不许"后到的赢"）/ **阈值写死**（`OCR_CONF_MIN=60` · `OCR_CJK_MIN=20` · `OCR_LATIN_MAX=0.4`，不做可调参数）。

实现 = `src/ocr.js`（`rotateImage90`）+ `src/convert.js`（`imageConvert` 重试；`ocrOnce` / `cjkLatin` / `ocrScore` / `poorOcr` + 阈值常量）。

| 编号 | 断言 | 标准 | 当前（基线 `347eaa5`，契约先红） |
|---|---|---|---|
| X1 | 行为级：首次差（conf 22）→ 须重试一次，且最终取更优 | `recognize` 次数 === 2 且 markdown === 第二次 | 🔴 红（先红实测次数 **1**）→ 🟢 绿 |
| X2 | **守护**：首次好（conf 88）→ **恰好 1 次** `recognize` | 次数 === 1 | 🟢 绿（"成本只在坏例上"） |
| X3 | 行为级：重试输入须是**旋转过**的图（位图宽高互换） | `b.w===a.h && b.h===a.w` | 🔴 红（先红实测只有 1 次尺寸记录）→ 🟢 绿 |
| X4 | **守护**：两次都差 → 取分高者、不崩、仍给 warnings | markdown === `garbage B`（25 > 20） | 🔴 红 → 🟢 绿 |
| X5 | **守护**：首次"中文 + 高拉丁占比"（= 会提示版面/方向）同样触发重试 | 次数 === 2 且取更优 | 🔴 红 → 🟢 绿 |

**先红 → 后绿**：**先红** = 只跑组 X → `tests 6 / pass 1 / fail 5`（X1/X3/X4/X5 红，X2 守护绿）；**后绿** = 全量 **246 tests / 244 pass / 0 fail / 2 skip**，产物 **120,988 B** / SHA `E5037CE1…`。
**产品路径三态实测（真 chromium + 真 OCR，本侧实跑）**：正立 CJK **118**（1288ms，不重试）/ **逆时针 90° CJK 7 → 重试后 118**（2263ms ≈ 1.75×，只在此坏例付费）/ 顺时针 90° CJK **118**（873ms，不重试）。
**副作用（已实测，用户拍板接受）**：坏例耗时约 **1.75×**。
**过程教训（登记）**：首次产品路径验证**被 Service Worker 的离线回退骗过**（本地服务未起来时 SW 用预缓存旧 `index.html` 顶上，页面正常但跑的是旧代码）→ 换端口（新 origin）后无干扰；另：最初的自造夹具（单行、22:1 长宽比）两个方向都失败，**差点得出"方向无关"的错误结论** —— 夹具保真度决定结论正确性。

### 契约组 Y — 大文件路径：产物守卫（先绿）+ 解析量/耗时（先红）（2026-09-18；设计见 `docs/大文件路径专项-验收判据-20260917.md`）

**背景**：为 **1000 行**的输出付了**全量解析**的代价 —— 桌面实测（真 Chromium，见 `docs/大文件路径专项-桌面实测-20260918.md`）：
`4-mid-large`（35.9 MB / 25 万行）**55–67 s** · `3-big`（47.4 MB / 43.6 万行）**69–99 s**，浏览器私有提交**增量 +6.4–7.4 GB**；
而 `1-small`（154 KB）与 `2-mid`（773 KB）**22–25 ms** 秒过（D4 指名口径下页面内存 **6–7 MB**）。

| 断言 | 内容 | 性质 | 当前状态 |
|---|---|---|---|
| **Y1** | 四档 md **字节 + SHA256** = 冻结基线（D1：产物逐字节不变） | **先绿守卫** | ✅ **绿**（小档基线 2026-09-18 沿用；**大档 2026-09-19 重冻**，见下「口径变更登记」） |
| **Y2** | `meta.scan.bytes ≤ 8 MB` 且两档差 ≤ 10%（D2，证明"与规模无关"） | **先红** | ✅ **绿**（2026-09-19 流式化落地；实测两档 `802,816 B`（Node）· 浏览器 3-big `851,968 B`，**差 0%**） |
| **Y3** | 25 万 / 43.6 万两档 `convert < 5000 ms`（D3） | **先红** | ✅ **绿**（2026-09-19 实测：浏览器 3-big **441–535 ms**、Node 44–85 ms；旧值 55–99 s） |
| **Y4** | 负对照：切回旧实现后 Y2/Y3 必红，随后完整还原 | 手工 | 已并入 **Linux 独立验收单**（做法见 §7） |

**冻结基线（2026-09-20 卡 007 再冻：三档变、一档不变）**：
`1-small` **61,990 B** / `0B48F22474116D5CEFC2D8A615D49C013DB0722F19794AB1409BC0555D8950EE` ·
`2-mid` **23,611 B** / `BD38B16DFE570EFA811819F4DB7E878EB1CB0E0A7B2BA3C35A5249FEB6661F04`（**一字不变**）·
`4-mid-large` **352,023 B** / `37FE03E76A13999D93B16B8E90B6013D82360F3CB8621D3337E2CE10A1B2F381` ·
`3-big` **352,027 B** / `2D064C9304FFBA22D33E373CE3C09F6CE6C153D6388FB683E778251290A0AF59`。

⚠️ **口径变更登记（2026-09-20，卡 007；用户 2026-09-20 拍板「没做先做」+ 给定"渲染层"口径）**：渲染层开始把
「有效数字 > 15 位」的数值字面量按 **15 位有效数字**显示（治浮点长尾，见 §2 **契约组 G9**）⇒ 三档产物字节变、**一档不变**：
`1-small` 66,140 → **61,990**（−4,150 B；窗口内 **266** 个长尾，**259 个在 C 列**）·
`2-mid` **0 B**（窗口内长尾 **0 个** ⇒ 天然干净对照）·
两档大档各 **−732 B**（窗口内长尾 3,039 个，但 **D/J/K ≈2,989 个是 17 位日期序列号 + 日期样式 ⇒ 走日期路径、不受影响**；
真正变的是 **R 列 50 个** —— ⭐ **正是卡 002 记的「50 行」**）。**旧值（66,140 / 352,755 / 352,759 与三个对应 SHA）保留作历史，勿再当基线引用。**
> **判据依据**：卡 007 的 baseSource 明写「**必答题 1 实测有长尾 ⇒ A5 改写为「新值 + 逐档说明」**」—— 本登记即该分支的落地；
> 且本卡**未改任何夹具**（`fileBytes` 四档全部不变），只改**显示**。

⚠️ **口径变更登记（2026-09-19，用户拍板「选项 1」；卡 002）**：两档**大夹具的原冻结值（`4-mid-large` 339,912 B /
`F90DCE36…` · `3-big` 352,029 B / `C2C41F5A…`）是「库回退路径」（`read-excel-file`）的产物** —— 撤 4 MB 护栏前
`sharedStrings` compSize 16.4 / 10.4 MB > 4 MB ⇒ 自解析路径 throw ⇒ 走库路径。**撤护栏（本卡明文要求）后大档改走
流式快路径**，而两条路径对这些文件**本来就不等价**：`4-mid-large` 表宽 **18 → 22 列**（1001 行里 **951 行前 18 列
逐字符相同**，另有 3 行在 20+ 列有真实数据）。
⚠️ **2026-09-19 更正（手机侧真机实测推翻我方初判 —— 留痕，旧说法作废）**：初判写成「快路径按 **`c@r` 列号归位**（2026-09-10 拍板、组 S3 守着）造成的**语义差异**」；**实测根因是旧路径丢数据** —— 夹具**声明** `<dimension ref="A1:R250001"/>`（18 列）而**实际存在 A–V（22 列）单元格，且 S–V 四列有真值**（实测首行 `DRIVECAPACITY=482547666944` / `FREESPACE=244628843904` / `USEDSPACE=458084823040` / `RESERVEDSPACE=5499568128`），**旧路径按声明宽度取宽 ⇒ 静默丢弃 4 列真数据**，快路径按实际单元格算宽 ⇒ **找回**。
⇒ 该变更是**修数据丢失**（方向正确），**不是"改口径"**（证据：`.私档/验证/手机实测-20260919-大文件流式化/`，交换仓 `phone/20260919-大文件流式化取证` = `3584783`）。
**流式化本身零字节变化**：等价性台（旧快路径（仅关护栏）vs 新流式）⇒ **四档 markdown 逐字节相同**（已证，见
`docs/任务台账.md` 卡 002 回执）⇒ 变的是**走哪条路径**，不是"读出来什么"。**旧值保留作历史，勿再当基线引用。**

⚠️ **夹具不入库**（35.9 MB / 47.4 MB；同 `real-cid-paper.pdf` 先例，2026-09-10 拍板）：默认目录 `.私档/传输-手机-20260918/`，
可用环境变量 **`DOC2MD_BIG_FIXTURES`** 覆盖；**缺失时本组整体跳过（不是失败）** ⇒ **CI 不会红，先红的可见性在本机 / 用户机**。
⚠️ **D4（内存）不在本组**：其指名口径 `performance.measureUserAgentSpecificMemory()` 需 `crossOriginIsolated`（COOP/COEP 响应头），
而 `tests/lib/server.mjs` 未开 ⇒ D4 按「**修后再测**」处理（小档地板已实测 6–7 MB，200 MB 目标站得住）。
⚠️ **Y1 基线是「本构建」产物**（含首行截断注释，L4c 形态①）：实测**跨环境确定** —— 手机侧旧构建产物 = 本基线**剥离首行注释后**逐字节相同（差 115 B，已完证）。

### 契约组 Z2 — 「保存」环节的原生/浏览器分流（卡 010；2026-09-20 契约先红）

来源：卡 008 的实测输入 —— **`A4 ❌ 红`**（Capacitor WebView 里 `<a download>` **静默无反应**；源码级根因 = `com/getcapacitor/**` 无 `DownloadListener`，`Download/` 里没有任何产品写出的 `.md`）+ **`A7 ✅`**（原生插件骨架 `Doc2mdNative.saveText` 已能写进 MediaStore.Downloads）。卡 010 用户拍板 **A**：**Web 侧 feature-detect**（行为差异写在 `src/` 里可读），**Web 行为不变**。

**口径（写死）**：
- 探测放**点击时**（不是加载时）：`window.Capacitor.Plugins.Doc2mdNative` 存在**且 `saveText` 是函数** ⇒ 走原生；否则 `<a download>`。**「插件对象存在」≠「能保存」**（负例 Z2-3 守着）。
- 接入两条**文本**路径：`downloadMd`（纯文本 .md）与 `downloadMdEmbedded`（图片内嵌 .md）；⚠️ **`downloadZip`（二进制 zip）未接** —— 插件目前只有文本写入（二进制需另立卡）。
- 原生保存失败**不静默**：状态栏报「保存失败：…」（「静默无反应」正是本卡要治的病）。

| 编号 | 断言 | 标准 | 当前 |
|---|---|---|---|
| Z2-1 | **Web 侧不变**：无 Capacitor ⇒ 点「⬇ 下载 .md」仍走 `<a download>`，产物含令牌 | download 事件 + 产物内容 | 🟢 绿（先红阶段即绿 = 回归闸基线） |
| Z2-2 | **APK 分支接线**：注入合法桩 ⇒ 调 `saveText({filename,text,mime})` 且**不再**产生 download 事件 | 桩收到 1 次调用 + 无 download | ✅ 绿（先红：`调用次数=0`） |
| Z2-3 | **负对照**：Capacitor 在但 `saveText` 不是函数 ⇒ 回落 `<a download>`，且**不调用任何插件方法** | download 事件 + 调用数 0 | 🟢 绿（先红阶段即绿） |
| Z2-4 | **一处真相（源码级）**：`src/ui.js` 与产物 `index.html` 里都可读到 `Doc2mdNative` / `saveText` / `Capacitor`；且探测**必须查 `typeof … === 'function'`** | 正则 + 字面量 | ✅ 绿（先红：`src/ui.js：读不到原生插件名 Doc2mdNative`） |

⚠️ **证据分层（卡面 A4 明文要求 —— 别把「CI 绿」当成「APK 通了」）**：
- **进 CI 的**：Z2-1 / Z2-3（Web 不变）+ **Z2-2（用桩模拟原生桥 = APK 逻辑进 CI 的唯一手段）** + Z2-4（源码级）。
- **只能真机的**：**卡 010 的 A1**（APK 里点「⬇ 下载 .md」⇒ 文件真落到用户可见位置，真机 `ls` 铁证）—— 本组**不替代**它。

**两相实测（本机 · 卡 010）**：**先红** = `node --test --test-reporter=tap --test-name-pattern="契约组 Z2" tests/contract_v1.test.mjs` → `tests 5 / pass 2 / fail 3`（exit 1；红的是 **Z2-4** 与 **Z2-2**）；**后绿** = 同断言全绿、全量 **302 / 300 / 0 / 2**（带夹具）/ **299 / 296 / 0 / 3**（干净检出，CI 侧待本轮推送对账）。

### 契约组 Z3 — 「保存」环节不再静默（卡 011；2026-09-20 契约先红）

来源：卡 010 §十一 的卡片材料（一手实测驱动：异机抽查中 NEO7「转换成功、点保存后状态栏一字未变、`Download/` 无文件」）。
⚠️ **只做「不再静默」，不修根因**（根因 = 那台机器上「插件为何未注册」**另立卡**，需那台设备 `logcat`）。

**口径（写死）**：两条分支都以 `isNativeRuntime()`（`Capacitor.getPlatform() !== 'web'`）为前提 —— 原生壳里 `<a download>` **静默无效**（卡 008 A4 反证 + 源码级 `com/getcapacitor/**` 无 `DownloadListener`）⇒ 「做不到」必须**明说**：
- **洞 A**：原生壳 + **探测不到插件** ⇒ 状态栏「保存失败：本机未加载保存插件，文件未保存」，**不回落**（回落 = 静默）；
- **洞 B**：原生壳点 **zip 主按钮** ⇒ 状态栏「本机（APK）暂不支持 zip 保存，请改用「🖼 下载 .md（图片内嵌）」」（替代路径本来就有，用户只是不知道）。
- 文案**表驱动**（`NATIVE_LIMIT`）；**Web 侧行为完全不变**（回归闸 = 组 Z2-1 / 组 I4，**旧断言一字未动**）。

| 编号 | 断言 | 标准 | 当前 |
|---|---|---|---|
| Z3-1 | 桩 `Capacitor={getPlatform:'android'}`（**无 Plugins**）⇒ 点「⬇ 下载 .md」出现可见提示且**无下载事件** | 状态栏含「未加载保存插件」+ `dl === null` | ✅ 绿（**先红**：`仍走了 <a download>`） |
| Z3-2 | 同桩 + 带图文档 ⇒ 点 zip 主按钮出现「暂不支持 zip」+「图片内嵌」且**无下载事件** | 正则 + 文案 + `dl === null` | ✅ 绿（**先红**：`zip 仍走 <a download>`） |
| Z3-3 | ⭐ **A8（第二轮）**：桩 **含 Plugins** 且 `saveText` **resolve** ⇒ 状态栏出现**成功回话**（**含文件名**）且无下载事件 | 含「已保存」+ 文件名 + `dl === null` | ✅ 绿（**先红**：`写入成功后状态栏无成功回话…实际="完成：共 1 个文件。"`） |
| Z3-4 | ⭐ **A9（第二轮）**：桩含 Plugins 但 `saveText` **reject** ⇒ 可见失败提示，且**不回落** `<a download>` | 含「保存失败」+ `dl === null` | 🟢 绿（**先红阶段即绿 = 免费绿**：实现早已覆盖，本条把它**钉成断言**） |

**两相实测（本机 · 卡 011）**：**第一轮** 先红 `tests 3 / pass 0 / fail 3` → 后绿全量 **305 / 303 / 0 / 2**（带夹具）/ **302 / 299 / 0 / 3**（干净检出，已由真 CI run 对账）；**第二轮（A8/A9）** 先红 `tests 5 / pass 3 / fail 2`（红在 **Z3-3**，Z3-4 免费绿）→ 后绿全量 **307 / 305 / 0 / 2**（带夹具）/ **304 / 301 / 0 / 3**（干净检出，已由真 CI run `35520749619` 对账）。

### 契约组 Z4 — 保存反馈必须落在**当前视口内**（卡 011 **第三轮** · `A10`；2026-09-21 契约先红）

来源：**用户真机实测打回**（卡面「修订记录 #2」原文）——「这个下载反馈不明显，**以保存那行字太小**，而且**我试了多个文件**，以保存那个**在最上面的文件的上方，根本看不见**，要**滑屏幕到最上面**才看得到那个小字」。

⚠️ **本组存在的原因 = 上一轮的判据不够**：`A8` 只写了「**可见**成功提示」—— **"可见"没定义相对谁**；而第二轮的验收样本恰好是**单文件**（页面短到不用滚）⇒ **条件绕过了缺陷**，于是"三面全绿"与用户的感受**同时为真**。⇒ 本组把「**可见**」口径钉死为「**落在 `window.innerHeight` 内**」，并且**必须先造出用户的现场**（多文件 + 滚到底）再断言。

**口径（写死）**：
- **载体**：`fixed` 定位的浮层（**Material Snackbar 形态**：底部 · 浮于内容之上 · 单行 · 自动消失 6000ms），**自己写不引库**（需求只有 1 成功 + 1 失败文案，候选库的可配置性一条都用不上；引库还要过 `licenses.md` + 交付面审计 + APK 体积三道门）；**样式写在 `src/ui.js` 里懒创建**，**不动 `src/template.html`**（那是两端共享模板，加原生专用 CSS = 动了 Web 的样式表）。
- **适用范围**：**只加在原生分支**。Web 侧浏览器**自带**下载完成反馈 ⇒ 再加一层 = 一个动作两条成功提示（用户原话「这样 web 不就有两个下载成功反馈了？」）。
- **字号下限**：≥ **14px**（用户抱怨的另一半是"字太小"）。
- ⚠️ **"可见"判据必须三层，缺一层就可能恒真**：元素**存在** → `display !== 'none'` → **`rect` 非零**（`display:none` 的元素 `getBoundingClientRect()` 全 0，只判"落在视口内"必过）。

| 编号 | 断言 | 标准 | 当前 |
|---|---|---|---|
| Z4-0 | ⭐ **现场前提**（不是结论）：3 文件 ⇒ 3 张卡 + 滚到底 ⇒ 旧载体 `#status` **已在视口外** | `actionRows === 3` + `scrollY > 0` + `statusInViewport === false` | ✅ 绿（**先红阶段即绿** —— 它是"现场成立"的证明；不成立则 Z4-1 退化成恒真） |
| Z4-1 | 原生桩 + 3 卡 + 滚到底 ⇒ 点**最后一张卡**的「⬇ 下载 .md」⇒ 提示必须**落在视口内** | `#toast` 存在 + `display≠none` + rect 非零 + `rect.top≥0 && rect.bottom≤innerHeight` + 文案含「已保存」 | ✅ 绿（**先红**：`视口内提示元素不存在 —— 反馈仍只在文档流顶部的 #status 里`） |
| Z4-2 | 提示**字号 ≥ 14px** | `parseFloat(getComputedStyle().fontSize) >= 14` | ✅ 绿（**先红**：同因） |
| Z4-3 | ⭐ **回归闸**：**Web 分支零变化** ⇒ 无 Capacitor 时点保存**仍走 `<a download>`** 且**不出现浮层** | download 事件存在 + `#toast` 不存在 | ✅ 绿（**先红阶段即绿** = 改动前的正确基线；比原 `A3` 闸更强） |
| Z4-4 | 原生桩 **reject** + 多文件 + 滚到底 ⇒ **失败回话同样落在视口内** | `#toast` 存在且 `fullyInViewport` + 文案含「保存失败」 | ✅ 绿（**先红**：`{"exists":false}`） |

**两相实测（本机 · 卡 011 第三轮）**：**先红** `node --test --test-reporter=tap --test-name-pattern="契约组 Z4" tests/contract_v1.test.mjs` → **`# tests 6 / # pass 2 / # fail 4`**（红在 **Z4-1 / Z4-2 / Z4-4**；**Z4-0 与 Z4-3 先红阶段即绿**，各自是"现场成立"与"Web 基线正确"的证明）→ **后绿**全量 **313 / 311 / 0 / 2**（带夹具）/ **310 / 307 / 0 / 3**（干净检出，按同一映射推得，待真 CI run 对账）。

**真机（第三级证据，2026-09-21 · vivo V2573A / Android 16）**：APK **18,965,761 B**，**从 APK 内读出的 `assets/public/index.html` = 137,866 B / `C06F654A…` = 仓库产物逐字节相同**；多文件现场 `cards=3` · `scrollY=1182` · `docHeight=1926` ⇒ **`statusInViewport=false`**（用户抱怨被复现）；**真实触摸**（`adb shell input tap 1080 2394`，打在**第 3 张卡**的按钮上）⇒ 浮层 `exists=true · position=fixed · fontSize=15 · rect={top:674,bottom:720} · fullyInViewport=true`，文案「✅ 已保存到「下载」：card011-a10-3.md」；**截图为证**（状态栏与页面内容同框，`.私档/验证/卡011-真机实测/a10-toast.png`）。


### 契约组 Z5 — `android/` 的 FileProvider **已退役**（卡 013 · `A1`；2026-09-21 契约先红）

来源：卡 012 §八 的 **`R-1`**（实测：`res/xml/file_paths.xml` = `<external-path name="." />` ⇒ 把**整个外部存储**暴露给 FileProvider，而**全仓无调用者**）。

**口径（写死）**：**唯一实现 = 删除** —— 删 `res/xml/file_paths.xml` + 删 `AndroidManifest.xml` 里的 `<provider>`。
⚠️ **为什么不是"收窄"**：既然**没有调用者**，则「真正需要的子路径」= **空集** ⇒ 收窄**没有判定目标**（独立审查者实测：全仓 `FileProvider|getUriForFile` 唯一命中就是那条声明本身）。
⚠️ **为什么要新开一组**：本项目**现有守卫都不覆盖它** —— `G1` 只管 `assets/public/index.html`；`G2` 只解析 `<uses-permission>` ⇒ **不补断言的话，删前删后都"绿"**，那不叫守。
⭐ **本组在 CI 也能跑**（这是它放在契约组而不是 `tools/` 的原因）：`android/` 的这两个文件是**入库**的，不依赖 APK / SDK / 构建产物。

| 编号 | 断言 | 标准 | 当前 |
|---|---|---|---|
| Z5-1 | `res/xml/file_paths.xml` **已删除** | `fs.existsSync === false` | ✅ 绿（**先红**：`file_paths.xml 仍存在`） |
| Z5-2 | `AndroidManifest.xml` 里**不再声明** `<provider>`（且不出现 `FileProvider` / `@xml/file_paths`） | 三处均为假 | ✅ 绿（**先红**：`manifest 里仍有 <provider> 声明`） |
| Z5-3 | ⭐ **现场前提**（先红阶段即绿）：**代码面零调用**（`android/app/src/main/java` + `src/` 范围内 `FileProvider`/`getUriForFile` 零命中） | 命中数 = 0 | ✅ 绿（**它是"可删"的依据**，不是结论） |

⚠️ **一条实现细节（本轮现场踩到，写进断言里防再犯）**：`Z5-2` **先剥掉 XML 注释再判** —— 删除处置**本来就该在原位留一句"为什么删"**，而那句话里必然出现 `<provider>` / `FileProvider` / `file_paths` 三个字面量；不剥注释的话，这条断言会被**自己的说明文字**绊红。（旁证：合并后的 manifest 里确实还有 `<provider>` —— 那是 **androidx 自动合并的 `InitializationProvider`**，与我们删掉的那条无关。）

**两相实测（本机 · 卡 013）**：**先红** `node --test --test-reporter=tap --test-name-pattern="契约组 Z5" tests/contract_v1.test.mjs` → **`# tests 4 / # pass 1 / # fail 3`**（红在 **Z5-1 / Z5-2** 与组壳；**Z5-3 先红阶段即绿** = 现场前提成立）→ **后绿**全量 **317 / 315 / 0 / 2**（带夹具）/ **314 / 311 / 0 / 3**（干净检出，按同一映射推得，待真 CI run 对账）。
...
| `sample-scale-td.pdf` | PDF（合成） | 契约组 **U9**（2026-09-16，P1 缺陷批）——**文本空间位移未乘 `Tm` 缩放**：页面级 `cm` 翻转 + `Tm = [0.05 0 0 -0.05 …]`（`Tf` 280 → 真实 14pt）+ **逐字 `TD` 推进**（dx=280=1em）+ **同一视觉行由两个文本对象拼成**（真机 WPS 导出形态：行1 = `ONE`(x 72/86/100) 接 `TWO`(x 114/128/142) ⇒ 期望 `ONETWO`；旧实现逐字推进 280 ⇒ 按 x 排序交错成 `OTNWEO`） | 1,193 B / SHA `78D67DA8…`；%PDF-1.4 合法，Helvetica，5×beginText（行1 两对象同 `Tm.f`）+ 20×setLeadingMoveText[280,0] | 字节锁（manifest）；纯拉丁（T-2 口径）；确定性生成 |
...
|---|---|---|---|
| T-1 | 耗时口径：规划文档「渲染 <500ms」 vs architecture「转换 <500ms」 | ✅ **已拍板**：convert() 外部计时 <500ms 为主口径；图片 OCR 冷启动（WASM/模型载入）按档位处理——**预热不计入** | C3/M2 断言保持 convert() 外部计时 <500ms；image 预热机制（预热钩子或分阶段计时，需 B 线 tesseract 封装配合）由 T4 落地，在此之前 image 冷启动红为契约预期。**2026-09-04 追加（T8′ 首载优化拍板，DD-14）**：预热钩子取消，语言包改为**同源懒加载**（`langs/`，lazy-init）——首次 OCR 冷启动含本地模型加载，按 T-1 档位豁免；「预热不计入」口径相应调整为「lazy-init 冷启动不计入」；断言数字与结构不变。**2026-09-19 追加（卡 002 · D 项，用户拍板 B′）**：把「预热后计时」**真正落地** —— image 用例同页两次转换：冷启动**只判防呆上限 ≤20,000 ms（非性能判据）**，**预热后**计时按 **`<500 ms` 主口径**断言（原 5000 ms 豁免窗口作废，保留作历史）。依据 = 2026-09-18 实测偶发超时 10,194 ms（Linux 侧控制变量证明与补丁无关；原做法只是把 flaky 留在门禁里）。**改口径 ≠ 悄悄改断言**：本条即登记 |
...
- **2026-09-20 xlsx 数值长尾的显示归一（卡 007：用户拍板「没做先做」+ 给定"渲染层"口径）**：目标 = `34.200000000000003` 这类**仅由 IEEE754 表示噪声造成**的长尾在**渲染层**恢复可读形态，**解析层保持忠实读出**、**真精度一律不截**。
  - **落点判定（卡面只钉"层"不钉文件）**：落在 **`src/xlsx.js` 的「值 → 文本」那一步**（`serialDateOrRaw`）—— 理由：markdown 产物就是"显示"，而"一个数值单元格该显示成什么"只有这一层知道（`convert.js` 只做汇总、`ui.js` 只渲染整篇 markdown，都看不到单元格）。**同一文件里解析与渲染是两段**：解析阶段 10 个函数逐字未动（A3 证据见 §2 契约组 G9）。
  - **实现**：`src/xlsx.js` 新增 `sigDigitsOf` + `formatNumericLiteral`（有效数字 > 15 且含小数点 ⇒ 15 位显示；**整数/≤15 位一律原样**），`serialDateOrRaw` 重接（日期样式仍走 `excelSerialToDate`）。
  - **先红 → 后绿**：新增**契约组 G9**（见 §2）。**先红** = `tests 7 / pass 3 / fail 4`；**后绿** = G9 7/7、全量 **297 / 295 / 0 / 2**。
  - **A5（口径变更登记）**：四档实测**有长尾** ⇒ 走「新值 + 逐档说明」分支，三档变一档不变 ⇒ 见 §2 契约组 Y 的「2026-09-20 卡 007 再冻」；**夹具零改动**（`fileBytes` 四档不变）。
  - **两条 lint 硬门禁（`.replace`/正则坑）**：首版 `sigDigitsOf` 用了 `/^(\d*)(?:\.(\d*))?.../`（嵌套量词）⇒ `sonarjs/super-linear-regex` **error**；改成线性扫描后，`.replace(/0+$/, '')`（开头不锚定）**又被判 error** ⇒ 最终**首尾零裁剪也手写扫描**（见 `DEV-NOTES` 卡 007）。
  - **A3 硬证据**：`git worktree` 取开工基线 `416e7d2` 做函数级切片比对 —— **10 个解析阶段函数 + 3 个渲染阶段函数 byte-identical**，只 `serialDateOrRaw` + 两个新增函数变。

- **2026-09-20 图片 alt 连续空白折叠（卡 006：用户实测「括号有问题」）**：目标 = `![alt](src)` **不跨行**（alt 里的空行会让渲染器断在空行处 ⇒ 页面剩裸 `![` / `](`）。
  - **实现**（两条入口 = 同一条链）：`src/docx.js` 新增 `docxCollapseWs`（`\s+` → 单空格 + 保留既有 trim）供 `docxAltFromName` 用；`src/html2md.js` `imgFrag` 对 alt 折叠空白（**只折叠不裁剪** —— 裁剪会动到无换行输入的字节）。
  - **先红 → 后绿**：新增**契约组 I2**（见 §2）。**先红** = `tests 7 / pass 1 / fail 6`；**后绿** = I2 **7/7**、全量 **290 / 288 / 0 / 2**。
  - **A5 产物判据（卡面要求先测再判）**：实测四档 `Y_TIERS` 夹具**全是 `.xlsx`**（backend `xlsx-self`）、**产物里 `![` 出现 0 次** ⇒ 本卡改动**不可能**触及它们；**离线核对台 4/4 逐字节 + 浏览器 Y1 绿**（`66140 / 23611 / 352755 / 352759` 与冻结基线全等）。
  - **两条入口都实测**（A6）：改前 `html2md` 侧 `<img alt="a\n\nb">` → `![a\n\nb](x.png)`（**中招**，字面换行 / `&#10;` / CRLF 三种写法都中）；改后 I2-4 五形态全绿。
  - **A10 端到端**：等价构造「15 图 + 每张 alt 含空行」⇒ **15/15 单行**、渲染 **15/15 `<img>`、零裸 `![`**（改前 0/6）。用户原件（论文）不入库。
  - **A11 新发现实测（与卡面 non-goal 的前提**不一致**，已更正登记）**：① 卡面称「docx alt 的 `]` 未转义」——**实测不成立**：`descr="A]B](https://…)&#10;C"` ⇒ 产物 `![A%5DB%5D(https://example.invalid) C](…)`，**`]` 已被 `%5D` 转义**（因为 docx 的 alt 最终经 `imgFrag`，转义就在那里）⇒ 该「另立卡」候选**应撤回或改写**；② `descr` 为 Word 样板句时它**就是 alt 文本**（`![AI 生成的内容可能不正确。 …](…)`）—— 属 C1 口径（文档自带属性优先），**本卡不动**，待拍板是否收窄。
  - **坑（一手，进 `DEV-NOTES`）**：① 我第一次量四档时**漏了 `withTruncationNotice` 组装**（直接调 `xlsxConvert`），四档**恰好各少 113 B** ⇒ 差点误判「基线漂移」（**卡面警告的正是这类误判**）；② 造夹具时**字面换行会被 XML 属性值归一化成空格** ⇒ 必须写 `&#10;`，否则红回路造不出来。
...
- **2026-09-19 契约数自动对账（卡 003：把最弱的字段变成最强的）**：`docs/BASELINE.json` 的 `contract.*` 由「文字溯源」升级为「**机器现算比对**」。
  ...
  - **按口径分列**：CI 只校 `ci_clean_checkout`（干净检出无 `.私档/` ⇒ 组 Y 整组 skip）；`local_with_fixtures` **只能在有夹具的机器上校**。2026-09-19（卡 003）本机实测两口径双双 PASS：`276 / 274 / 0 / 2` 与 `273 / 270 / 0 / 3`（**未改任何应然值**）；**2026-09-20（卡 005）新增组 G8 后两口径各 +7 ⇒ `283 / 281 / 0 / 2` 与 `280 / 277 / 0 / 3`，双双对账 exit 0**（**这两个值后来被真实 CI run `35458379084` 的对账步确认**）；**2026-09-20（卡 006）新增组 I2 后两口径再各 +7 ⇒ `290 / 288 / 0 / 2` 与 `287 / 284 / 0 / 3`，双双对账 exit 0**；**2026-09-20（卡 007）新增组 G9 后两口径再各 +7 ⇒ `297 / 295 / 0 / 2` 与 `294 / 291 / 0 / 3`，双双对账 exit 0**（`BASELINE.json` 由人工同步 —— 守卫查不出自洽但陈旧的契约数）；**2026-09-20（卡 010）新增组 Z2 后两口径再各 +5 ⇒ `302 / 300 / 0 / 2` 与 `299 / 296 / 0 / 3`**（带夹具侧本机升权实跑 exit 0；干净检出侧经真 CI run `35515740260` 对账确认）；**2026-09-20（卡 011）新增组 Z3 后两口径再各 +3 ⇒ `305 / 303 / 0 / 2` 与 `302 / 299 / 0 / 3`**（带夹具侧本机实跑 exit 0）；**同日卡 011 第二轮（`A8`/`A9`）组 Z3 再 +2 ⇒ `307 / 305 / 0 / 2` 与 `304 / 301 / 0 / 3`**（带夹具侧本机实跑 exit 0；干净检出侧经真 CI run `35520749619` 对账确认）；**2026-09-21（卡 011 第三轮 · `A10`）新增组 Z4 后两口径再各 +6 ⇒ `313 / 311 / 0 / 2` 与 `310 / 307 / 0 / 3`**（带夹具侧本机实跑 exit 0 **且 `--from-tap` 对账 exit 0**；干净检出侧经真 CI run `35588293123` 对账确认）；**2026-09-21（卡 012）不变量**（纯新增 `tools/` 守卫 ⇒ 契约数不动）；**2026-09-21（卡 013 · `android/` 收尾）新增组 Z5 后两口径再各 +4 ⇒ `317 / 315 / 0 / 2` 与 `314 / 311 / 0 / 3`**（带夹具侧本机实跑 exit 0 + `--from-tap` 对账 exit 0；干净检出侧按同一映射推得，待真 CI run 对账）。
...
- **2026-09-19 大文件流式化批（卡 002：P1 流式 + A9/A10 预检提示）**：目标 = 大 xlsx 峰值内存由 **O(文件)** 变 **O(窗口)**（设计见 `docs/大文件路径专项-验收判据-20260917.md`）。
  ...
  - **口径变更（唯一对外可见处，用户 2026-09-19 拍板「选项 1」）**：两档大夹具改走快路径后表宽 **18 → 22 列** ⇒ **大档 Y1 基线重冻**（登记见 §2 契约组 Y）；**小档逐字节不变**；等价性台证明**流式化本身零字节变化**（旧快路径 vs 新流式 ⇒ 四档 markdown 逐字节相同）。
  - **实测**：浏览器 `3-big` **441–535 ms**（旧 55–99 s）· 解析量 **802,816 B**（≤ 8 MB、两档差 **0%**）· 小档 66,140 B / 23,611 B 与原基线一致 · A9/A10 浏览器实证：提示在 **t=149 ms** 画出、转换 441 ms 完成（提示不进产物）。
  - **口径（T-1 B′）**：image 冷启动改为「只判防呆 + **预热后**按 `<500 ms` 计时」（见 §6 拍板点 T-1 追加条）。
  - **回箱结果（2026-09-19 晚；两份回执均逐文件核验）**：
    - **Linux 独立验收（弱独立：换机 + 换环境 + 换数据通道）**：组 Y **三绿** · 契约 **276 / 274 / 0 / 2**（与期望逐项一致，`✖` 0 条）· 组 C 预热后 **41 / 43 ms** · lint 0-0 · metrics **21 / 502 / 超限 0 / 0.4%** · pwa **48/48** · OCR **PASS(93)** · `meta.scan.bytes` 两档均 **851,968 B（差 0%）** · **`HEAD^{tree}` == `59fc644^{tree}`**（五笔逐笔 tree 相同）。
      ⚠️ **负对照未取得断言级红**：本机 5.78 GiB 内存跑不下旧实现的大档（3-big 渲染进程被内核 OOM），改用**谓词级现场量**逐条证伪：md **339,912 ≠ 352,755** · **无 `meta.scan`** · **70,026 ms ≥ 5,000 ms** · 3-big 树峰值 **5,346 MB → OOM**（形态与单据预期不同，已如实登记）。
    - **手机侧真机（`V2573A` / Android 16）**：两档**都能产出**且**指纹与我方重冻基线逐字节相同**（`96127CB7…` / `519D58AA…`）· 夸克 **609 / 742 ms**（修前 286.8 s）· 整包 PSS 峰 **1.13 / 0.86 GiB**（修前 12.63 GiB）· **转换边际 +131 MB**（÷解压后 361.0 MiB ≈ 0.36×，修前 ≈35.5×）· **A10 提示条截图实证**（361.0 MB / 最坏约 90 秒）。
  - ⚠️ **两条新登记（**未拍板、未实施**，均超出卡 002 范围）**：① **浮点显示**：夹具 XML 里存的就是 `34.200000000000003`，新路径**忠实读出** ⇒ 50 行可读性下降（旧路径做过数值归一）；若要恢复须在**渲染层**格式化，**不是解析层**。② **A10 提示条在秒级时代读不到**：渲染正常（有截图）但批次内页面 ≈7 Hz 重排；三选一 = 固定视口 sticky / 只在预计 >N 秒时显示 / 移除。
  - ⚠️ **一条判据口径（Linux 侧发现，需拍板）**：同一次运行「**保留量**」修前 +6.4–7.4 GB → 修后 **+0.2 MB**（强证脱钩）、「**窄窗口**（convert 内）」**+468 MB 达标**、「**宽窗口**（含 47.4 MB 装载）」**+646 MB 超 500 MB** ⇒ **内存判据必须写死窗口定义**（建议采用"转换边际：基线→峰"；落地位置 = `docs/大文件路径专项-验收判据-20260917.md` D4）。

- **2026-09-16 P1 批（PDF 文本空间位移未乘 `Tm` 缩放 → 顺序错乱；组 U 续号 **U9**）**：来源 = **手机侧真机实测**（钉钉收到的 WPS 导出 PDF，9 页通知：字符全对、顺序错乱、**零警告**）+ 电脑侧算子级独立取证（`.私档/项目/复盘/20260916-P1根因-电脑侧.md`）。
  现象与物证：手机侧产物与电脑侧本环境重跑**逐字节相同**（10,561 B / `C344E9C530F9…`）⇒ 缺陷确定性复现。
  根因（**实测**，非推断）：旧实现把**文本空间**位移量当**用户空间**累加 —— 真机 PDF 用 `TD`（dy≡0）**3229 次**、`Td` **0 次**、`Tm.a ∈ {0.03, 0.045, 0.05}`、`Tf` 虚高 20×；对照 Chromium 打印 PDF 的 `Tm.a` 恒 **1**、用 `Td`。**两版中间假设都被自己的实测否决**：「翻转是根因」（手机侧对照实验已推翻）·「`cm` 逐对象变化」（实测 showText 处 CTM 去重 = **1**）·「同一行混用缩放」（实测 **0/82** 行混用）。
  口径（用户 2026-09-16 拍板「**浅修** + 合成样例入库 + 插队最前」）：**有效字号 = `Tf × hypot(a,b)`**；`TD`/`Td`/`TL`/`T*` 的位移与 leading 分别乘 `hypot(a,b)` / `hypot(c,d)`；`BT` 复位缩放（规范：文本矩阵复位为单位阵）；`Tm` 的 e/f 是用户空间平移，**不缩放**。实现 = `src/pdf.js`（5 处，全部带注释）。
  提交链：`62f3f84`（先红：`sample-scale-td.pdf` 1,193 B / `78D67DA8…` + U9）→ `9b6e9c7`（实现 + 产物重建）。
  **两相实测（captain 升权实跑）**：**先红** = 组 U `tests 11 / pass 9 / fail 2`，U9 实际输出 `OSTNEHWECIOORNDDLLIINNEE` —— 暴露**两层**后果：① 行内 x 交错（推进 280 而非 14）② **字号未随缩放致行分组容差 140 > 行距 30 ⇒ 三个视觉行并成一组**；**后绿** = 组 U **11/11**、全量 **256 tests / 254 pass / 0 fail / 2 skip**、lint **0**、metrics 17 文件 / **361 函数** / 超限 0 / 0.5%、pwa-audit **48/48**、verify:ocr **PASS(93%)**；产物 **121,229 B / `9EA390B955BBA9C4E62BD5A58DD78073635575F4779572494C89897D36710A11`**（旧 120,987 B / `14773E5F…`）。
  **等价性台（两层，用户硬要求「既有正常 PDF 逐字节不变」）**：**函数级**（`.tmp/equiv-pdf.mjs`，before/after 两版 `src/pdf.js`）= 纯函数 25 场景 + **12,992** 参数扫掠 + 600 随机（LCG 固定种子）+ 全 BMP `isCjkChar` → **0 差异**；`tests/data` 11 份 PDF 的 `pdfPageRuns` 逐字段对比 → **既有样例 0 差异**（唯二差异 = 新样例本身：`fontSize 280→14`、`x 72/352/632→72/86/100`）。**产物级**（`.tmp/equiv-pdf-product.mjs`，真产物 + 真 `convert()` × 19 份语料）→ **A 组 16/17 逐字节相同**（10 契约样例 + 2 Chromium 对照 + 3 软著 PDF + OCR 文本）；B 组 WPS 乱码 → 与渲染页**逐行对齐**。
  **唯一例外（用户 2026-09-16 拍板接受）**：`real-cid-paper.pdf` —— 与 WPS **同族**（`Tm` 缩放 8–31、`Td` 0 / `TD` 6,586 次、`Tf=1`），**老输出本就含交错串**（`上多海种质要量素管共理同科实学现研质究量院`），新输出分行正确（`质量链管理理论` / `研究综述V` / `金国强刘恒江 （上海质量管理科学研究院…）`）；**C2 判据两版均通过**（CJK 4,105 → 4,109、占比 61.9%、三子串命中）⇒ 判定为**同一缺陷的第二个受害者**，非回归。
  **两条教训（已提炼）**：① **只查 `includes` 的断言抓不到顺序错乱** —— C2 对上述交错输出照样判绿（手机侧 §9-2 的经验在本侧独立复现）；② **「参考实现与待测实现输出相同」不等于都对** —— 第一版参考实现矩阵复合顺序写反（`mul(translate, Tlm)`），与 doc2md **逐字相同**，靠**渲染页目视**这一外部基准才识别出来。
  **登记（未做，非本批引入）**：逐字 PDF 的**拉丁/数字**被 ruleB 逐字空格打散（如 `2 0 2 6`）—— CJK 有规则C 抑制、拉丁/数字没有，留待后续拍板。
  **独立验收（Linux 侧，2026-09-16 12:36–12:52）—— 五项全绿，已通过**：真 chromium（未回退 msedge）· **V-1** 契约 `256 / 254 pass / 0 fail / 2 skip`（28.31s）+ 组 U **11/11**（与本侧逐项一致）· **V-2 负对照（决定性）**：`git checkout 62f3f84 -- src/pdf.js` + 重建 → 组 U `11 / 9 / 2`，`not ok` = U9 + 组壳，U9 消息 `行内顺序错乱（期望 ONETWO…）："<!--page1/1-->OSTNEHWECIOORNDDLLIINNEE"` —— **与本侧实测逐字相同**，还原后 **11/11** · **V-3 等价性 A 组** `相同 10 · 变化 1 · 缺件 0 · 违规 0`，退出码 0（10 份 normalized 参考值**逐条一致**；B 组 `sample-scale-td.pdf` `FE8955077108` → `1AEE3AD15645` = 51 → 68 字符）· **V-4 渲染基准**：用 pdf.js 渲染路径（`pdftoppm` 缺失，按纪律不装包）自造 PNG → 目视三行 `ONETWO / SECONDLINE / THIRDLINE` 顺序正确 ⇒ **期望值确非自证** · **V-5 换数据**：自造 `Tm 0.03/0.045 + 整词 Tj` 的 PDF（与样例参数不同）→ 旧产物 `ALPHAGAMMAEPSILONBETADELTAZETA`（三行拍平成一行）→ 新产物**三行正确**，无回归。
  **Windows 侧核验（回箱后实测）**：交付 **34/34 项字节 + SHA256 全相符**（合计 423,267 B）；TAP/日志原文直取核心判据复算；**两张渲染 PNG 由本侧亲自目视确认**（`ONETWO/SECONDLINE/THIRDLINE` 与 `ALPHA BETA/GAMMA DELTA/EPSILON ZETA`）。
  ⚠️ **计划外发现（Linux 侧 §7.1）**：**负对照态产物 ≡ 线上 v0.1.8 发布产物**（`120,987 B / 14773E5F…F3F178`）⇒ U9 的红**等价于「P1 缺陷在线上 v0.1.8 中真实存在」**，且反证构建链跨平台可复现（同源码 → 同产物）。
  **Linux 侧夹具保真度教训（§7.2，已采纳）**：自造件初版渲染**上下颠倒 + 镜像** —— 抄了 `Tm` 负缩放却没抄配套的 `1 0 0 -1 0 842 cm`（样例靠「cm 翻转 + Tm 负缩放」两次抵消才正常）⇒ **夹具必须先渲染目视确认「它本身视觉正常」再拿去比结论**。
  **未验证项（保留登记）**：U1 未重跑 `npm ci` · **U2 未独立复算「函数级台」**（该台脚本未随单提供）· U3 未复核 7 份私有语料（含 `real-cid-paper.pdf` 的 A 组例外拍板）· U4 V-4 用 pdf.js 渲染路径替代 `pdftoppm` · U5 未重复取样 · U6 自造件仅 1 份、未覆盖真机中文 WPS 形态。

- **2026-09-15 S4 判据门批（组 S 续号 S4-8..S4-14；阈值 2/10 → **3/4**；用户 2026-09-15 拍板）**：来源 = 收口批留下的 C4/C10/C13 未修 finding（小 nonAscii 基数下门放行 → 小文件被 gb18030 整篇改写）+ **决议6 附加条件①/② 的夹具缺口前置**（先补夹具/守卫网，再动门）。
  口径（用户 2026-09-15 拍板，**取代决议6 的候选⑤ `fffd>=5`**）：`fffd >= 3 && nonAscii > 0 && fffd*4 >= nonAscii`。**为什么不是 5**：三套阈值 × **17 例实测矩阵**（脚本 `.tmp/s4-gate-probe.mjs`，t12 对抗台重建件 + 仓库 F/S4 既有例 + 极短 GBK 边界）——现门 2/10 = **14/17**（破 C4/C10/C13）；候选⑤ 5/4 = **13/17**（**打破仓库既有 F6 短 GBK「hello world 你好」fffd=3 → 回归乱码**）；本次 3/4 = **16/17**。⚠️ **决议6 附加条件① 点名要纳入的是 F1/F2，实测其 fffd=6 反而安全；真正的破例落在没被点名的 F6 与 E2/E3** —— 教训：守卫清单要按**语义类别**取全（短 GBK / 极短 GBK / 微损坏 UTF-8），不能只列点名的几条。
  边界（如实登记，S4-14 锁行为）：单个坏字节最多产 1 个 U+FFFD ⇒ **fffd=2 与「2 字节真 GBK 单汉字」签名相同、无法区分** → floor=3 选择保住「UTF-8/ASCII + 2 处损坏不被整篇改写」，代价 = **2 字节单汉字 GBK 不再自动回退**（E1）。如需兜底须另加手动编码选择（**v1 范围外**）。
  **夹具形态**：S4-8..S4-14 全部**页内现造、不入库样例**（与 S4-5/6/7 同规格）。⚠️ **t12 原始字节不可得**（生成脚本在 `.tmp/qa-t12/`，已清理）→ **重建件**（判据值同量级非同值，明细见 §2 组 S 表下注）。
  **勘误**：决议 R1 与 HANDOFF §3 A5.3 曾记「**C8（8 B 真 GBK）**」—— t12 原始输出为 **C8 = S4-5 型 10,796 B + 2 截断**（fffd=2 → 期望保持 UTF-8）；「8 B」是 F1/F2 的量级（同步修正 DEV-NOTES 与 HANDOFF）。
  提交链：`33ed402`（先红断言：组 S 续号 S4-8..S4-14，+90/−0）→ `14d8544`（`src/sniff.js` 两常量 + 注释；产物重建）。
  **官方两相（captain 升权实跑，Windows / Node 24 + 系统 Edge 回退）**：**先红** = 只跑组 S → `tests 33 / pass 28 / fail 5`（S4-11/12/13/14 ✖ + S4 组壳；守卫 S4-8/9/10 ✔），其中 S4-12 实际输出 **`"浣犲ソ涓栫晫浣犲ソ涓栫晫浣犲ソ涓栫晫  鞍"`** —— 与 t12 原始记录**逐字一致**；**后绿** = 全量 **253 tests / 251 pass / 0 fail / 2 skip（50.5s）**、组 S **33/33**、`npm run lint` **0**、metrics 17 文件 / **361 函数** / 超限 0 / 0.5%、pwa-audit **48/48**、verify:ocr **PASS(93%)**；产物 index.html **120,987 B / SHA `4A15BA5135D1F7849553BE468BD8E1EF3A43D89D99F6FD8C69E20F1F7979EEA6`**（旧 120,988 B / `2EE82D47…1CBB`；bundle 82,513 → 82,512，−1 字节自洽）。
  **独立验收（Linux 侧，2026-09-15 19:38–19:41）—— 六项全绿，已通过**：真 chromium（未回退 msedge）· 契约 **253 / 251 pass / 0 fail / 2 skip（25.8s）** + 组 S **33/33**（与本侧逐项一致）；**V-2a 负对照**（切回 `2/10`）→ 组 S `33 / 28 pass / 5 fail`，红 = `not ok 11/12/13/14`（S4-11/12/13/14）+ 组壳、守卫 S4-8/9/10 绿；**V-2b 负对照（决定性）**（切到候选⑤ `5/4`）→ 全量 `253 / 249 pass / 2 fail`，红 = **F6**（`not ok 7`，消息 `输出含替换字符 U+FFFD："hello world ���"（短 GBK 中文替换占比 <30% 未触发兜底）`）+ 组 F 壳 → **「候选⑤ 会打破 F6」在本侧独立成立，无需重评阈值**；`iconv` 真 GBK **3/3 字符串全等**（+`python3` 复证）→ 手抄码点表未被证伪；产物 build 前后均 `120,987 B / 4A15BA51…9EEA6`（构建幂等 + 跨平台逐字节一致）；终态 `git status --porcelain` 0 行、`main` 指针未动。
  **Windows 侧核验（回箱后实测）**：交付 **25/25 项字节 + SHA256 全相符**（23 件产出 = **186,251 B** 合计，+ 被验对象 `src/sniff.js` / `tests/contract_v1.test.mjs`），`index.html` 单独实测同值；核心判据**直取 TAP 原文**复算，非采信转述。
  ⚠️ **计划外发现（Linux 侧 F-1，产品级事实）**：**负对照态 A 精确等价于线上发布的 v0.1.7**（`FFFD_MIN=2` / `FFFD_RATIO=10` + 产物 `120,988 B / 2EE82D47…1CBB` 逐字节一致）⇒ 这三类静默错（C4/C10/C13）**在线上 v0.1.7 中真实存在，本批修复尚未上线** —— 发布节奏由用户拍板（本记录只陈述事实与推断链）。
  **未验证项（保留登记，不编根因）**：U1 未重跑 `npm ci`（沿用既有 `node_modules`）· **U2 未独立复算「三套阈值 × 17 例」矩阵**（`.tmp/s4-gate-probe.mjs` 未入库 → 本侧只独立验证 `2/10` 与 `5/4` 两端点）· U3 未审阅 `b8311ff` 文档回填内容 · U4 未重复取样（本批均确定性断言）· U5 未评估 S4-14 边界（2 字节单汉字 GBK）的真实场景频率。
  **附带登记**：**F-2**「尺寸相同不能当不变量」再实例（`3/4` 与 `5/4` 同为 `120,987 B`、哈希不同：`4A15BA51…` vs `633B5F2F…`）；**F-3** Linux 侧采集脚本自身缺陷（`grep` BRE 下 `\+` 是量词 → 改 `-cF` 后命中），**属脚本缺陷、断言条件未改**。

- **2026-09-15 独立验收（v0.1.6 + v0.1.7 合并，Linux 侧）—— 六项全绿，已通过**：真 chromium 契约 **246 / 244 pass / 0 fail / 2 skip**（与本侧逐字一致；原始日志第 304–311 行）；真机样张 **4/4 达标**（CJK 438 / 268 / 65 / **276**）；**方向重试三态均可读（正立 / 逆时针 90° / 顺时针 90° 各 101.0%）**；成本 **1.30×**（无 2× 翻倍 ⇒「成本只在坏例」）；**负对照：切回 v0.1.6 实现后同一逆时针夹具 CJK 0/84 = 0.0%（全篇乱码）→ v0.1.7 为 101.0%** —— **因果锁定**（负对照态产物 = `119,699 B / 3D044AA0…` = v0.1.6 产物哈希）；预缩放矩阵 3000×2000→**1500×1000**、800×600→**原样**（只缩不放）+ `tessedit_pageseg_mode: '3'` 显式确认;metrics 17 / **361** / 超限 0 / 0.5%。
  Windows 侧实测核验 **29/29 文件字节 + SHA256 吻合**；**U1 已关闭**（补件后探针 5/5 · 5/5 · 6/6 · 5/6，且替代路径「产品路径产出 vs 用户手上 `.md`」**去空白后 100.00% 一致**）；**U2 由本侧结案**（真机旋转页的旋转方向属 AUTO **支持侧** → 首次即可读、**不触发重试**，故 v0.1.6/v0.1.7 结果相同是**正确行为**）；U3–U6 保留。

- **2026-09-15 v0.1.7 批（新增组 X：OCR 方向重试；用户拍板"不加 10MB / 只重试一次 / 取更优 / 阈值写死"）**：来源 = **产品路径实测**发现 tesseract 的 AUTO **只认一个旋转方向**（同一 10 行中文夹具：顺时针 90° → CJK 118/123；逆时针 90° → CJK 7 全篇乱码），而用户的真机旋转照片恰好落在"支持"的那一侧（故 v0.1.6 复测通过）。
  口径：触发 = **与 `ocrWarnings` 同一口径**（提示即行动 ⇒ 成本只在坏例）；动作 = **顺时针旋转 90° 重试一次**（`rotateImage90`，**不加 OSD 资产**）；**只重试一次 / 取更优（写死打分 `conf − 100 × 拉丁占比`）/ 阈值写死**。
  实现：`src/ocr.js`（`rotateImage90`）+ `src/convert.js`（重试 + `ocrOnce` / `cjkLatin` / `ocrScore` / `poorOcr` + 常量）。
  **两相实测（Windows / Node 24 + 系统 Edge 回退；captain 升权实跑）**：**先红** = 只跑组 X → `tests 6 / pass 1 / fail 5`（X1/X3/X4/X5 按设计红，X2 守护绿）；**后绿** = 全量 **246 tests / 244 pass / 0 fail / 2 skip（46.7s）**，lint 0 warning、metrics 17 文件 / **361 函数** / 超限 0 / 0.5%、pwa-audit **48/0**、verify:ocr **PASS(93%)**；产物 **120,988 B / SHA `E5037CE1718D2B7FECE5C570722861C18314F285B90866D05590FA34867F7970`**。
  **产品路径三态（真 chromium + 真 OCR，本侧实跑）**：正立 118（1288ms，不重试）/ **逆时针 90°：7 → 118**（2263ms，重试生效）/ 顺时针 90° 118（873ms，不重试）。
  **独立验收：待 Linux 侧。**

- **2026-09-15 v0.1.6 批（新增组 W：OCR 输入质量；用户拍板三项口径）**：来源 = **真机 4 张样张实测网格**（用户提供 4 组「.jpg + 其手机端 .md」）→ 定位 **tesseract.js 隐式默认 PSM 6（SINGLE_BLOCK）** 是「图旁正文整段丢失」「90° 旋转页整页乱码」的根因；**PSM 3（AUTO）在 4 张上从不最差，且是旋转页唯一可读配置**（**旋转无需 OSD 资产**）；长边缩放至 ≈1500px 使探针 2/5 → **5/5** 且提速 30–60%。
  口径（用户 2026-09-15 拍板）：① 长边 >1500px 预缩放（只缩不放）② 显式 PSM 3 ③ 质量信号 = 低置信度 + 高拉丁占比（**仅中文场景**判「版面或方向问题」）。
  实现：`src/ocr.js`（`OCR_PSM` / `MAX_OCR_EDGE` / `prepareOcrImage`）+ `src/convert.js`（接入 + `ocrWarnings` 增判据）。
  **两相实测（Windows / Node 24 + 系统 Edge 回退；captain 升权实跑）**：**先红** = 只跑组 W → `tests 7 / pass 3 / fail 4`（W1 `params=[]` / W2 `{w:3000,h:2000}` / W4 `warnings=[]` 三条按设计红，W3/W5/W6 守护绿）；**后绿** = 全量 **240 tests / 238 pass / 0 fail / 2 skip（51.3s）**，`npm run lint` **0 warning**、metrics 17 文件 / **353 函数** / 超限 0 / 0.5%、pwa-audit **48/0**、verify:ocr **PASS(93%)**；产物 index.html **119,699 B / SHA `3D044AA0E741EED97C111EDC39658320049D64FDE622E03AB79C8AFBB4993D4C`**（blob `4adb0c820a76fa40f44c03ab4a1befe16afdf2e5`）。
  **连带修正**：组 R 的 R3 假 worker 补 `setParameters` 空实现（测试桩随接口升级，**R3 断言未动**）。
  **独立验收：待 Linux 侧（真 chromium + 真 OCR 端到端）—— 未完成前本批不算验收通过。**
...
- **2026-09-11/12 规范符合性 S4+S5 收口批（S4-5..S4-7 / S5-4 / S5-5 先红后绿；AgentTeams `doc2md-s4s5` 续跑）**：来源 = 首轮交叉审查的 3 条 finding（S4-R1 全文 ≥2 处零散 U+FFFD 触发**整篇 gb18030 mojibake**；S5-R1 同前缀混排漏改；S5-R2 属性值含 `>` 漏改）→ 用户 2026-09-11 拍板「**修完再发**」（口径 A′）。
  ...
  **未修 finding（全 low，登记为已知边界，留后续加固批）**：① ~~C4/C10/C13 —— nonAscii 基数小时门可通过、小文件被 gb18030 改写（**t9 前后行为一致，非本批引入**；收紧时须不破坏 C1/C2/C8 判别）~~ ✅ **已闭环（2026-09-15 收尾批：阈值 2/10 → 3/4；组 S S4-8..S4-14；先红 `33ed402` → 后绿 `14d8544`）**——且**实施值与决议6 的候选⑤（`fffd≥5`）不同**：候选⑤ 经 17 例实测会打破 F6 短 GBK，故取 floor=3（见 §7 2026-09-15 条）；② T13-L1 注释/CDATA 内未闭合 `<w:dstrike>` 与后续真实配对元素跨边界配对（**新旧输出逐字节相同 = 自 S5 既有**）；③ T13-L2 dstrike 非法嵌套内层漏改；④ T13-L3 畸形输入下配对分支 O(n²)（2k 未闭合 73 ms）。
...
### 契约组 Y（2026-09-18 新增）—— **2026-09-19 转绿**，红绿如实
...
- ⚠️ **本组在 CI 上整体跳过**（夹具不入库）⇒ **不要用 CI 绿灯推断 Y2/Y3 已达标**；判定必须在**有夹具的机器**上跑。两个口径（2026-09-20 卡 007 后）：**本机带夹具 = `297 / 295 pass / 0 fail / 2 skip`** vs **CI 干净检出 = `294 / 291 / 0 / 3`**（见 `docs/BASELINE.json`）。
