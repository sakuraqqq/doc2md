...
**冻结基线（2026-09-19 重冻大档；小档沿用 2026-09-18 原值）**：
`1-small` 66,140 B / `A8A777C0A5CF13EDD6FF1EB824609A03F49CE230A3C56B10D553B6A912BF9F91` ·
`2-mid` 23,611 B / `BD38B16DFE570EFA811819F4DB7E878EB1CB0E0A7B2BA3C35A5249FEB6661F04` ·
`4-mid-large` **352,755 B** / `96127CB737C520C6BDFC34BA0D3E8942BA60B16DCA3800A6C64C8658917A120D` ·
`3-big` **352,759 B** / `519D58AAA964F7D893D629B2604A4D25CB2CBB4E40EFB678EBC5C97EA3F9281D`。
...
- **2026-09-19 契约数自动对账（卡 003：把最弱的字段变成最强的）**：`docs/BASELINE.json` 的 `contract.*` 由「文字溯源」升级为「**机器现算比对**」。
  ...
  - **按口径分列**：CI 只校 `ci_clean_checkout`（干净检出无 `.私档/` ⇒ 组 Y 整组 skip）；`local_with_fixtures` **只能在有夹具的机器上校**。2026-09-19（卡 003）本机实测两口径双双 PASS：`276 / 274 / 0 / 2` 与 `273 / 270 / 0 / 3`（**未改任何应然值**）；**2026-09-20（卡 005）新增组 G8 后两口径各 +7 ⇒ `283 / 281 / 0 / 2` 与 `280 / 277 / 0 / 3`，双双对账 exit 0**（**这两个值后来被真实 CI run `35458379084` 的对账步确认**）；**2026-09-20（卡 006）新增组 I2 后两口径再各 +7 ⇒ `290 / 288 / 0 / 2` 与 `287 / 284 / 0 / 3`，双双对账 exit 0**（`BASELINE.json` 由人工同步 —— 守卫查不出自洽但陈旧的契约数）。
...
- ⚠️ **本组在 CI 上整体跳过**（夹具不入库）⇒ **不要用 CI 绿灯推断 Y2/Y3 已达标**；判定必须在**有夹具的机器**上跑。两个口径（2026-09-20 卡 006 后）：**本机带夹具 = `290 / 288 pass / 0 fail / 2 skip`** vs **CI 干净检出 = `287 / 284 / 0 / 3`**（见 `docs/BASELINE.json`）。
