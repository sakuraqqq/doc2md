...
| 断言 | 内容 | 性质 | 当前状态 |
|---|---|---|---|
| **Y1** | 四档 md **字节 + SHA256** = 冻结基线（D1：产物逐字节不变） | **先绿守卫** | ✅ **绿**（基线 2026-09-18 冻结） |
| **Y2** | `meta.scan.bytes ≤ 8 MB` 且两档差 ≤ 10%（D2，证明"与规模无关"） | **先红** | ❌ **红**（`meta.scan` 字段尚不存在） |
| **Y3** | 25 万 / 43.6 万两档 `convert < 5000 ms`（D3） | **先红** | ❌ **红**（实测 55–99 s，超 11–20×） |
| **Y4** | 负对照：切回旧实现后 Y2/Y3 必红，随后完整还原 | 手工 | 待实现后补 |

**冻结基线（2026-09-18；小档 2× 复跑、大档 3×·2× 复跑，字节与哈希全同）**：
...
`4-mid-large` 339,912 B / `F90DCE360064A3FA9375DA58ADCA5888C78CB24CDC889BB7629B6EB159174F17` ·
`3-big` 352,029 B / `C2C41F5AFB9105E91616ADD9A686545C00F68936D25E86E672DA99FC29C7ADCB`。
...
### 契约组 Y（2026-09-18 新增）—— 红绿如实

- **Y1 🟢 绿**（四档冻结基线见 §2 契约组 Y）· **Y2 🔴 红**（无 `meta.scan` 字段）· **Y3 🔴 红**（55–99 s ≫ 5 s）· **Y4 待补**（实现后做负对照）。
- **转绿路径**：**解析止损 —— 够 1000 行即停**（`xlsx-self` 与**库回退两条路径都要改**；只改一条等于没治），并新增**内部** `meta.scan = { bytes, rows }`
  —— ⚠️ **只进 meta，绝不写进用户下载的 `.md`**（2026-09-18 用户拍板）。
- **Y4 负对照做法**：临时切回「库回退全量 + 不记录 scan」⇒ **Y2、Y3 必须变红**；随后**完整还原**，并以 **Y1 确认产物回到基线字节**（负对照态若产物变了，说明切回不干净）。
- ⚠️ **本组在 CI 上整体跳过**（夹具不入库）⇒ **不要用 CI 绿灯推断 Y2/Y3 已达标**；先红/后绿的判定必须在**有夹具的机器**上跑。
