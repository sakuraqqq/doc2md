...
export function apply(ctx, config) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    // PATFIX-2026-09-04-delegated-children-full-catalog
    // Delegated children (subagents / AgentTeams members) get the full catalog
    // from turn one: they spawn as fresh sessions with no warm-up turn, so
    // first-turn core-set trimming would leave them without the tools their
    // task needs (GitHub/browser/web/fs-search), and the workspace AGENTS.md
    // tool routing would be unmet. origin 'subagent' or a parent session both
    // identify a child; top-level sessions have neither.
    const header = session.header ?? {}
    if (header.origin === 'subagent' || header.parentSession !== undefined) {
      return { ...assembled, sections, contexts: [] } // full catalog for children
    }
