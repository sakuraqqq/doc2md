...
> **完整设计见 `..\doc2md-调度\总览.md`** —— 本节只写**你必须知道的关系**，**不复述全文**（复述必然漂移）。
...
| 东西 | 谁写 | 你 |
|---|---|---|
| 任务卡 `..\doc2md-调度\队列\<NNN>-*.md` | 调度线 | **只读**（读不受工作区限制，直接读） |
| 调度线台账 `..\doc2md-调度\台账\*` | 调度线 | 只读 |
| 代码 `src\` · 产物 `index.html` · `docs\` · `tests\` · `.私档\` | **你** | **独占** |
| `git push` / `npm publish` / `gh release` | **用户本人** | 只准备到可执行 |

⚠️ **你写不进调度工作区**（沙箱强制：写域 = 会话自己的 cwd 及其下），**也不要为它申请升权** —— 那不是限制，那是这个设计的支点。

### 你的工作循环

1. **读卡**：`..\doc2md-调度\队列\` 里 `状态：待接` 的卡
2. **接卡前判三件事**：判据可判？边界清楚？自包含？—— **不合格退回调度线并说明缺什么，不自己补**（自己补 = 把两个角色又合回一个）
3. **七步**：定基线 → 先红 → 实现 → 门禁 → 证据 → 回写 → 报告
4. **回执写在自己这边**（`docs\HANDOFF-主开发线.md` / `docs\DEV-NOTES.md`）—— **不要回写调度线的队列**
5. **报告必须带「坑 / 根因 / 防再犯」**（没坑写"无"）—— 一手复盘归你；**只写在会话里没落盘的不算复盘**
6. **一次只做一张卡**；**卡上没有的活不做** —— 哪怕它就在眼前、只有一行

**技能**：`task-execution`（本工作区 `.dsh\skills\`，rank 100 自动发现）—— 接卡、七步、门禁四件套、**开 AgentTeams 的「花钱闸门」**（默认不开；只在真并行 ≥3 / 需独立评审环 / 需长时试错时开）。

### 出外部单也是你的事（**不是调度线的**）

| 平台 | 出单 | 回箱 |
|---|---|---|
| **Linux** | `.私档\linux\任务-<主题>\`（交接单**三件套 = 期望值 + 判据 + 预期例外**） | `.私档\linux\<YYYYMMDD>-<批次>\` |
| **手机** | `orders-<日期>-<主题>.bundle` 进交换仓 / LAN 页面上传 | 手机回 `delivery-*.bundle` → **你手工并入**交换仓 → 归档 `.私档\验证\` |

（平台细节见 `..\doc2md-调度\平台\{windows,linux,phone}.md`；分工与配额见本仓库 `docs\HANDOFF-主开发线.md` §3.5）

### 复盘分层（**别全揽，也别全推**）

| 层次 | 归谁 | 落在哪 |
|---|---|---|
| 执行级（每张卡踩的坑） | **你** | 卡的回执 + `docs\DEV-NOTES.md` |
| 纪律沉淀（同一坑 ≥2 次 ⇒ 工具 / SKILL / 断言） | 判断归调度，**实施归你** | 调度出「沉淀卡」→ 你做 |
| 批次级（排序、收口、下一批） | **调度线** | 调度侧 `台账\复盘.md` |
| 路线级（转正 / 发版 / 范围） | **用户拍板**（材料调度出） | `HANDOFF` / `docs\RELEASE.md`（**你落盘**） |
