...
| 图片（PNG/JPG/…） | tesseract.js OCR | ✅ | eng+chi_sim（LSTM 量化，语言包 `langs/` 同源懒加载）｜**v0.1.6 起**：长边 >1500px 的大图先等比缩小再识别（只缩不放，更准也更快），OCR 版面模式固定为「自动版面分析」——**横着拍/竖着拍的整页文档都能正确读出**（实测整页旋转 90°：修复前全篇乱码 → 修复后正常）｜**已知限制（真机验收 2026-09-15）**：① **手写内容读不出**（手填数据表全败）；② **照片里的表格不保留表格结构**（值大多能读出，但不输出 GFM 表格）；③ **行内公式与示意图内部标注**读不出；④ 形近字偶有误识（如「覆盖」→「才盖」）；⑤ **关键数字请核对原图**——实测个案同一张样张 8 处数值全错；拍摄质量（光照/反光/倾斜/分辨率）直接决定准确率 |
