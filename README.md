...
| 图片（PNG/JPG/…） | tesseract.js OCR | ✅ | eng+chi_sim（LSTM 量化，语言包 `langs/` 同源懒加载）｜**已知限制（真机验收 2026-09-15）**：**OCR 结果不要用于数字/表格内容** —— 实测个案：同一张实拍样张的 **8 处数值全部识别错误**（而正文汉字基本可读）；拍摄质量（光照/反光/倾斜/分辨率）直接决定准确率，**关键数据请核对原图，或优先用带文字层的电子文档** |
