...
> **演示动图**：本工具核心操作（拖放 / 点击选择文件 → 转换 → 复制 / 下载 .md）均为浏览器直觉操作，暂不提供演示 GIF（2026-09-04 拍板）。真实转换示例见上方 `assets/screenshot.png`。

## 为什么做

- 市面上的转换工具大多要上传文件、注册账号或装 Python 环境；MarkItDown 是 CLI/服务端思路，网页版体验友好但基本都走云端。
- 大多数「文档转 Markdown」网页版要么在服务器转（有隐私风险），要么依赖 CDN（断网即废）。
- **与同类工具的实测对比**（同批样例、输出逐字符比对，判定准则先声明后判分）：MarkItDown → `docs/doc2md-vs-markitdown-对比报告-2026-09-13.md`；pdf-inspector（MIT，浏览器 WASM 路径）→ `docs/doc2md-vs-pdfinspector-对比报告-2026-09-14.md`。结论：**取值三方一致**，差异集中在段落与标题的组织方式（docx 表头语义 / xlsx 值保真 / 图片离线 OCR 三项本项目占优；**PDF 段落边界与标题识别登记为我方待改进项**）。
...
| PDF | pdf.js + tesseract.js OCR | ✅ | 文本层直取；扫描页/无文本层自动 OCR 降级（eng+chi_sim）｜**注意：OCR 需 http 服务（线上/本地）或在线打开；file:// 直开双击下 OCR 不可用——但文本层页不受影响（质量门未过的页保留原文本层 + 提示，v0.1.2），扫描页跳过并提示**——其余格式（PDF 文本层/DOCX/XLSX/TXT·HTML）file:// 双击可用｜已知限制：PDF 表格/竖排布局拍平为文本行（文本层无结构信息），数字字符偶有间隙误判（如 `0103401 2`）不影响可读性；连续空格折叠为单个（ASCII art/代码缩进丢失——第七轮 §2.5）；竖排/旋转文本未计算变换矩阵，run 定位与空格判定可能偏差（第七轮 §2.6）；多栏文档同行混排未做栏切分（第七轮 §2.1，v2 候选）；**图片型 PDF（PPT 导出为图片/扫描件，无文字层）自动走 OCR——中文词间空格已合并（2026-09-09 起），但仍有误识字，建议优先用保留文字层的导出方式** |
...
| 图片（PNG/JPG/…） | tesseract.js OCR | ✅ | eng+chi_sim（LSTM 量化，语言包 `langs/` 同源懒加载）｜**v0.1.6/v0.1.7 起**：长边 >1500px 的大图先等比缩小再识别（只缩不放，更准也更快）；OCR 版面模式固定为「自动版面分析」，**且结果不理想时自动把图顺时针转 90° 重试一次、取更好的那个** —— **横拍/竖拍的整页文档两个方向都能读出**（实测 10 行中文页：正立 / 顺时针 90° / 逆时针 90° 三态均命中 118/123；坏例耗时约 1.75×）｜**已知限制（真机验收 2026-09-15）**：① **手写内容读不出**（手填数据表全败）；② **照片里的表格不保留表格结构**（值大多能读出，但不输出 GFM 表格）；③ **行内公式与示意图内部标注**读不出；④ 形近字偶有误识（如「覆盖」→「才盖」）；⑤ **关键数字请核对原图**——实测个案同一张样张 8 处数值全错；拍摄质量（光照/反光/倾斜/分辨率）直接决定准确率 |
...
> **手机上的浏览器选择与三条已知限制（真机验收 2026-09-15 / 09-17 实测）**：**不推荐 Chrome** —— 它的文件选择器在应用声明了 `accept`（含图片类型）时会**丢掉「文件」入口**，只能从相册/图片里选，无法选 PDF/DOCX/XLSX（系统自带浏览器实测正常，可正常进入文件管理选任意类型）。三条限制：① **部分定制 Android 系统的 Chrome/Edge 没有「文件」入口**（成因同上，换用系统自带浏览器即可）；② **部分国产定制 ROM 的文件选择器不支持多选** —— 此时请逐个添加文件（应用本身支持多文件，逐个添加同样可用）；③ **聊天 App 内置浏览器（宿主 WebView）**：从微信/钉钉里点开链接时，选择器由宿主提供、**通常只给「照片和视频」** —— 微信内置**只有相册**；钉钉内置**「上传文件」是灰色不可点的**，且在钉钉内置里点「下载 .md」**可能没有任何反应**（真机实测）。⇒ 遇到 ③ 请点右上角 **「⋯ → 在浏览器打开」**，转到系统浏览器/夸克后再选文件（**QQ 内置实测正常**，可作对照）。**另**：Web Share（系统分享面板）在实测的各家手机浏览器上均不可用，故导出仍以「复制 / 下载 .md」为准。
