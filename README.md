...
> **手机上的浏览器选择与两条已知限制（真机验收 2026-09-15 实测）**：**不推荐 Chrome** —— 它的文件选择器在应用声明了 `accept`（含图片类型）时会**丢掉「文件」入口**，只能从相册/图片里选，无法选 PDF/DOCX/XLSX（系统自带浏览器实测正常，可正常进入文件管理选任意类型）。两条限制：① **部分定制 Android 系统的 Chrome/Edge 没有「文件」入口**（成因同上，换用系统自带浏览器即可）；② **部分国产定制 ROM 的文件选择器不支持多选** —— 此时请逐个添加文件（应用本身支持多文件，逐个添加同样可用）。**另**：Web Share（系统分享面板）在实测的各家手机浏览器上均不可用，故导出仍以「复制 / 下载 .md」为准。
