...
> 基线：**HEAD `10930d9`**（**已推送**，`origin/main` 与之同步；v0.1.5 批 5 提交已在远端） · 契约：**231/231**（全量 233 tests / 231 pass / 0 fail / 2 skip，skip = real-cid-paper 第三方样例不入库；历史链：S4/S5 后绿 179/179 → 第八轮 212/212 → v0.1.4 提交 C 后 230/230 → **v0.1.5 新增组 R R3 后 231/231**） · 门禁全绿（lint 0/0 · metrics 文件 17 / 函数 349 / 超限 0 / **重复率 0.5%** · PWA 48/48 · OCR 93% PASS · 产物 `index.html` **118,263 B** / `200D004B…E4DB0`） · **跨平台已验（2026-09-15 两轮）**：产物逐字节相同、门禁逐项一致；v0.1.5 批另经**真 OCR 中文图端到端 + 负对照**独立验收（四项全通过，详 `docs/DEV-NOTES.md` 2026-09-15 两节） · 工作区：与版本库同目录
...
2. **路径口径已定、实施排 v0.2**：候选⑤（S4 门 `fffd>=5 && fffd*4>=nonAscii`，附加条件见决议 §3/§6）、T2 1904 只补 warning、T4 `%` 收紧版并入（`+` 不纳入）。
3. **C1/C2/C8 原始夹具缺失**（t12/t13 对抗台编号在仓库内只有引用、无定义）→ **补夹具入库**；且**候选⑤ 实施后若原始夹具下 C8 表现不同，候选⑤ 必须重新评估**（决议附加条件②）。
