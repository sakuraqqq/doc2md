...
1. **v0.1.4 缺陷批（已拍板，两批提交）**：**提交 A** = 批 1（A1 翻转 Tm 行序 / A2 缺 `TL`(36) 算子）+ 批 2（A3 等宽代码围栏 / A4 叠印去重）；**提交 B** = 批 3（B1 `BM*`·`GIF8*` 前缀误判 / B2 `%PDF` 文本误判 / B3 大写 `META` 漏检 / C1 docx alt 取 `descr`）+ T3 两项（脏数字实体闭环登记 + C1 映射表 23 项参数化断言）。**每批跑门禁，绿了再进下一批。**
