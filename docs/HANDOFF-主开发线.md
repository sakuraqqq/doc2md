...
> 发布（**最新，待用户执行**）：**v0.1.8 发布准备就绪（2026-09-15）** —— 修复 S4 编码判据门三类静默错（线上 v0.1.7 已知受影响，见下）；契约 **255/253/0/2** · lint 0 · metrics 17/**361**/超限 0 · PWA 48/48 · OCR PASS(93%) · 产物 **`120,987 B / 14773E5F…F3F178`** · **隐私 & 版权审查：通过**（落盘 `docs/RELEASE.md` v0.1.8 节）· Release 专用说明 `docs/release-notes-v0.1.8.md` · 发布动作（用户终端）：`git push origin main` → `git tag v0.1.8` → `git push origin v0.1.8` → **网页发 Release**（从下拉选中已有 tag）。
...
> ⚠️ **线上 v0.1.7 已知受影响（2026-09-15 独立验收附带发现 F-1）**：`src/sniff.js` 的 S4 门在 v0.1.7 中为旧阈值 `2/10`，与 Linux 侧负对照态 A **逐字节等价**（产物 `120,988 B / 2EE82D47…1CBB`）⇒ **C4/C10/C13 三类静默错（小 nonAscii 基数下小文件被 gb18030 整篇改写）在线上真实存在**；修复（`3/4`）已在 main（`14d8544`）· **用户 2026-09-15 拍板发 v0.1.8**（发布准备就绪，见上方「最新」行）—— ⚠️ **v0.1.8 上线前，线上仍受影响**。
