...
> 基线：**HEAD `ef6bdba`**（main；S2/S3 规范符合性 + metrics 口径 + 文档漂移对齐 + gitignore 批） · 契约：**165/165**（**2026-09-11 全门禁复跑实测**，45.6s 零回归；减脂批前为用户终端权威跑 153/153） · 门禁全绿（lint 0/0 · metrics 超限 0 / 重复率 0.6% · PWA 48/48 · OCR 93% PASS · 产物 diff 0） · 工作区：与版本库同目录
...
已发布 **v0.1.2**（tag `v0.1.2` = `f5aed38`）。tag 后 main 继续前进到 **`ef6bdba`**（第七轮批 + v0.1.3 首提交准备 + 真实样例批 OCR 中文空格合并 + **减脂批：metrics 超限 23 → 0** + **公开仓库合规清扫** + **规范符合性 S2 删除线 / S3 表格列位置** + **metrics 假绿修复与口径定案** + **embed-bline 护栏 / 隐患处置落盘** + **文档漂移对齐（权威源 / 数字回填）** + **`.gitignore` 收口**），**不重打 tag**（用户拍板）。**2026-09-11 全门禁复跑（本会话升权实跑）**：契约 **165/165**（45.6s 零回归）、PWA **48/48**、OCR **93% PASS**、build + `git diff --exit-code index.html` **exit 0**、`eslint src/**` **0w/0e**、metrics **文件 16 / 函数 323 / 超限 0**；**重复率 0.6%**（口径 = **只度量 `src/` + `tools/`，`tests/` 排除**，2026-09-10 用户拍板；原记 4% 系 metrics 读旧报告的假绿，工具已修）。**截至实测时远端 `origin/main` == `812dbd6`；其后本会话新增 `ef6bdba`（`.gitignore`）待你 push。**
