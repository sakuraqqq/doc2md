...
1. **v0.1.6（待做，阻塞项：等样张）**：① **显式设置 PSM** —— 实测现状 = tesseract.js 默认 **PSM 6（SINGLE_BLOCK）**，与用户手机输出**逐字相同**（三条独立证据）；改 **PSM 3（AUTO）** 后同一张图探针命中 **11/16 → 13/16**，**救回整句正文 + 图注**。② **大图预缩放**（canvas，长边 ≤ ~2000px）—— 25–50% 缩放实测 **14–15/16** 且**提速 25–45%**。③ **借鉴 pdf-inspector「先分类再决定策略」**（**只借鉴思路、不抄代码**）：按**版面类型**选 PSM，而非所有页用同一个值；我们已有等价物的另一半（PDF 逐页质量门 → 逐页 OCR 降级），缺的正是"按版面选策略"。
