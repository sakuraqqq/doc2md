...
> 基线：**HEAD `5683b05`**（v0.1.3 发布准备批：版本号 + 注释对齐 + 组 T + 部署 smoke；其后仅文档提交） · 契约：**180/180**（**2026-09-11/12 S4/S5 批 + 收口批两相实测**：先红 168/6 → 后绿 174/174（46.6s）→ 收口批先红 21/5 → **后绿 179/179（46.0s）** 零回归；减脂批前为用户终端权威跑 153/153） · 门禁全绿（lint 0/0 · metrics 超限 0 / 重复率 0.6% · PWA 48/48 · OCR 93% PASS · 产物 diff 0） · 工作区：与版本库同目录
...
5. **规范符合性剩余（S4/S5 已于 2026-09-11 闭环）**：S1 补断言锁死 · S2 残余 CSS `text-decoration: line-through`（是否支持待拍板）
