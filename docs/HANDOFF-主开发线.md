...
> 基线：**HEAD `2b2a0eb`（main；S2 批产物提交，其后一个提交为本文件所在文档批）** · 契约：**159/159**（S2 批后实测；减脂批前为用户终端权威跑 153/153）· 工作区：与版本库同目录
...
已发布 **v0.1.2**（tag `v0.1.2` = `f5aed38`）。tag 后 main 继续前进到 **`2b2a0eb`**（第七轮批 + v0.1.3 首提交准备 + 真实样例批 OCR 中文空格合并 + **减脂批：metrics 超限 23 → 0** + **公开仓库合规清扫** + **规范符合性 S2 删除线修复**），**不重打 tag**（用户拍板）。当前契约 **159/159**、PWA 48/48、`eslint src/**` **0w/0e**、metrics 超限 **0**、重复率 4%。**本地领先远端若干提交待 push。**
...
2. **S3 表格列位置错位**——① XLSX 稀疏单元格：`parseCellAttrs` 不取 `r` 列号 → `A2/C2` 变 `| A2 | C2 |  |`；② HTML `colspan` 未展开 → `| wide | c |  |` → **待修**（拍板点：列号补空位 / 乱序处理 / colspan 语义）
