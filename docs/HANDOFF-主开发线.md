...
> 基线：**HEAD `03e3a03`（main；第七轮批 + v0.1.3 首提交准备后）** · 契约：**150/150**（本会话实跑）· 工作区：与版本库同目录
...
已发布 **v0.1.2**（tag `v0.1.2` = `f5aed38`）。tag 后 main 继续前进到 **`03e3a03`**（第七轮批 `5b9bb72`→`8a21e4f` + v0.1.3 首提交准备：footer 版本 + lockfile），**不重打 tag**（用户拍板）。当前契约 **150/150**、PWA 48/48、lint 31w/0e、metrics 超限 23 全绿；远端 main = 本地 HEAD（已 push）。
