...
> 基线：**HEAD `a010bc8`**（**v0.1.7 代码提交**；随后一提交为文档回填） · 契约：**244/244**（全量 246 tests / 244 pass / 0 fail / 2 skip，skip = real-cid-paper 第三方样例不入库；历史链：… → v0.1.5 组 R R3 后 231/231 → v0.1.6 组 W 后 238/238 → **v0.1.7 组 X 后 244/244**） · 门禁全绿（lint 0/0 · metrics 文件 17 / 函数 **361** / 超限 0 / **重复率 0.5%** · PWA 48/48 · OCR 93% PASS · 产物 `index.html` **120,988 B** / `E5037CE1…7970`） · **v0.1.6/v0.1.7（2026-09-15）**：OCR 输入质量（显式 PSM 3 / 大图预缩放 / 版面质量信号）+ **方向重试**（质量差 → 顺时针 90° 重试一次，取更优）· **独立验收待 Linux 侧**（真 chromium + 真 OCR 端到端；**v0.1.6 那份交接单的产物哈希已被 v0.1.7 取代**，见 §3 G） · 工作区：与版本库同目录
