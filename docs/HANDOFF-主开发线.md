...
> 基线：**以现场实测为准（本文不缓存 HEAD / 远端 SHA —— 见下「易变量规则」）** · v0.1.7 已发布、已推送（tag `v0.1.7` = `9d2cd9d`） · 契约：**251/251**（全量 253 tests / 251 pass / 0 fail / 2 skip，skip = real-cid-paper 第三方样例不入库；历史链：… → v0.1.5 组 R R3 后 231/231 → v0.1.6 组 W 后 238/238 → v0.1.7 组 X 后 244/244 → **2026-09-15 S4 判据门批后 251/251**） · 门禁全绿（lint 0/0 · metrics 文件 17 / 函数 **361** / 超限 0 / **重复率 0.5%** · PWA 48/48 · OCR 93% PASS · 产物 `index.html` **120,987 B** / **`4A15BA51…9EEA6`（2026-09-15 S4 判据门批；v0.1.7 发布版本 = 120,988 B / `2EE82D47…1CBB` —— 页脚串等长 → 字节不变、哈希必变）**） · **v0.1.6/v0.1.7（2026-09-15）**：OCR 输入质量（显式 PSM 3 / 大图预缩放 / 版面质量信号）+ **方向重试**（质量差 → 顺时针 90° 重试一次，取更优）· **独立验收已通过（Linux 侧六项全绿；负对照 0% → 101%）**（真 chromium + 真 OCR 端到端；**v0.1.6 那份交接单的产物哈希已被 v0.1.7 取代**，见 §3 G） · 工作区：与版本库同目录
