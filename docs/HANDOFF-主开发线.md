...
> 基线：**HEAD `03bad6c`**（**v0.1.6 代码提交**；随后一提交为文档回填 —— 两者均**待推送**） · 契约：**238/238**（全量 240 tests / 238 pass / 0 fail / 2 skip，skip = real-cid-paper 第三方样例不入库；历史链：S4/S5 后绿 179/179 → 第八轮 212/212 → v0.1.4 提交 C 后 230/230 → v0.1.5 组 R R3 后 231/231 → **v0.1.6 组 W 后 238/238**） · 门禁全绿（lint 0/0 · metrics 文件 17 / 函数 **353** / 超限 0 / **重复率 0.5%** · PWA 48/48 · OCR 93% PASS · 产物 `index.html` **119,699 B** / `3D044AA0…4DB0`） · **v0.1.6（2026-09-15）**：OCR 输入质量三修（**显式 PSM 3** / 大图预缩放 / 版面质量信号）—— 根因 = tesseract.js **隐式默认 PSM 6**；**独立验收待 Linux 侧**（真 chromium + 真 OCR 端到端，未完成前本批不算验收通过） · 工作区：与版本库同目录
