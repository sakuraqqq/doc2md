...
2. **S4 判据门已实施（2026-09-15 收尾批）；T2/T4 仍排 v0.2**：候选⑤（`fffd>=5 && fffd*4>=nonAscii`）**经 17 例实测被否决**（会打破 F6 短 GBK）→ **实取 `fffd>=3 && fffd*4>=nonAscii`**（契约组 S **S4-8..S4-14**；先红 `33ed402` → 后绿 `14d8544`；详情见 `tests/CONTRACT.md` §7 2026-09-15 条）；T2 1904 只补 warning、T4 `%` 收紧版并入（`+` 不纳入）**仍排 v0.2**。
