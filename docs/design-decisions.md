...
## DD-17 · 构建管线 String.replace `$` 模板陷阱（产物与源码脱节）— 2026-09-06 用户抓包

- **现象**：用户比对 `src/docx.js`（`m.block ? '$$'…'$$'`）与线上产物（`"$"…"$"`）——源码块级公式 `$$` 在产物中变成 `$`；106 项契约测试全绿、CI 构建一致（一致地错）。
- **根因**：`tools/build.mjs` 注入 bundle 用 `template.replace(MARKER, '<script>\n' + bundleJs + '\n</script>')`——**String.replace 的字符串替换器会展开 `$` 模板**（`$$`→一个字面 `$`、`$&`/`$1` 等魔法）→ bundle 中所有 `$$` 被折叠成 `$`。t24 之前 bundle 无 `$$` 未暴露；t24 引入块级 `$$` 后，**所有产物均为折叠版**，而 L3 断言（`$[^$\n]*$` 宽容提取）恰好匹配折叠结果 → 全绿未察觉。
- **拍板/修复**：替换器改为**函数形式** `template.replace(MARKER, () => ...)`（函数返回值不做模板展开）——一行修复 + 注释留证；重建产物后 106/106 全绿、产物 `$$` 核实（grep）。
- **修复后验收**：产物 grep `m.block ? "$$"…` ✓；106/106（用户机）；build.mjs 语法/幂等保持。
- **防再犯**：① build.mjs 注释警告；② 凡「字符串替换注入代码」一律函数式替换器；③ 检验产物与源码一致性用「源码关键串 grep 产物」对照（本次即此法发现）；④ 升级「源码↔产物对照检查」为构建自检（下版候选）。
- **全系列确认（用户 2026-09-06 补）**：`$` 魔法不止 `$$`——还有 `$&`（整个匹配）/`` $` ``（前缀）/`$'`（后缀）/$1-$99（捕获组）——函数式替换器对**全系列**免疫（一类修复，非逐个补丁）；源码/产物扫描确认业务代码仅 `$$` 序列（合法公式边界），无其他魔法序列受历史影响。
- **复盘**：构建管线层 bug 比业务代码更深——契约/CI 全绿不保证产物=源码；「版本对比」类检查（用户 grep 源码/产物）是最有效防线。
