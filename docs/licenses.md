...
| 4 | mammoth | github.com/mwilliamson/mammoth.js（master） | **1.12.2**（已内联） | **BSD-2-Clause** | ① 官方仓库 `LICENSE` 原文（browser 直读）："Copyright (c) 2013, Michael Williamson…" 的 BSD 2 条条件文本；② 本地 npm 包 package.json `"license": "BSD-2-Clause"`；③ **版本证据（2026-09-17 实测）**：`npm pack mammoth@1.12.2` 解包后与 `vendor/mammoth.browser.min.js` **逐字节一致**（SHA256 相同）—— 注意 bundle 内 `VERSION="1.13.1"` 是**内嵌的 underscore**，不是 mammoth 自身版本（曾一度误判，见 DEV-NOTES 2026-09-17） | ✅ 可商用；保留版权声明与许可文本 |
| 5 | read-excel-file | gitlab.com/catamphetamine/read-excel-file（npm 同源） | 本地 5.8.7 | **MIT** | 本地安装包 package.json `"license": "MIT"`（作者 catamphetamine）；**版本证据（2026-09-17 实测）**：`npm pack read-excel-file@5.8.7` 解包后与 `vendor/read-excel-file.min.js` **逐字节一致**（SHA256 相同） | ✅ 可商用，保留版权声明 |
...
| 7 | tesseract.js-core | npm: tesseract.js-core（github.com/naptha/tesseract.js-core） | 本地 **6.1.2**（wasm 单文件内联 ×2：lstm / simd-lstm） | **Apache-2.0** | 本地安装包 package.json `"license": "Apache-2.0"`；**版本证据（2026-09-17 实测）**：`npm pack tesseract.js-core@6.1.2` 解包后与 `vendor/tesseract-core-{lstm,simd-lstm}.wasm.js` **逐字节一致**（SHA256 相同）—— ⚠️ 本行原记 **6.0.0 系陈旧**（vendor 实际跟的是 lock 的 6.1.2），本次依赖体检交叉校验抓出并订正 | ✅ 可商用；附 Apache-2.0 文本与声明 |
