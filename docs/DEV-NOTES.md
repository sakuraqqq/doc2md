...
- **线上当前状态 + 已拍板处置**：线上仍是 main 构建（**121,406 B / `18EA70E4…`**，页脚 `v0.1.9`）—— 改触发口径**不会**回滚已部署内容。**用户 2026-09-17 拍板：接受现状，等下次发版（推新 tag）自然覆盖**（不手动 dispatch；依据 = 该批行为零变更，两层等价性台已证，用户无感）。**推论**：v0.1.9 观察期内如需把线上产物与某个哈希对齐，基准是 **`18EA70E4…`**（main 构建），不是发布物 `F80E8626…`。
...
## 2026-09-17 大 xlsx 边界实测（用户 WizTree 全量导出 → 切件）：护栏内仍冻死 117 秒

### 缘起与输入
用户拿 WizTree 导出做「**接近 50MB**」的边界测试（原始导出 107.27 MB / 1,048,843 行，超护栏），在 Linux 侧自行切成 `WizTree_20260917150849_cut.{csv,xlsx}` 后反馈**超时**。我方实测核验：

| 输入 | 实测 |
|---|---|
| 原始导出 | **112,478,909 B = 107.27 MB**；`sheet1.xml` 解压 **703.8 MB**、`sharedStrings.xml` **213.9 MB**；**≈1,048,843 行 / 1,847 万单元格（≈17.6 列/行）**；CSV 版 **851.81 MB / 2,286,593 行**（⇒ **xlsx 导出早已被 Excel 的 1,048,576 行上限截过**） |
| 用户切件 xlsx | **49,716,718 B = 47.41 MB ✅ < 50MB**；`sheet1.xml` 291.1 MB 解压、`sharedStrings.xml` **87.4 MB** 解压 |
| 用户切件 csv | 161.76 MB（doc2md 只把 .csv 当**纯文本**，不做表格 —— 见 README 支持域） |

### 实测结果（真产物 + 真 msedge；`.tmp/convert-file2.mjs`，带外部超时 + **主线程心跳探针**）
| 输入 | convert 耗时 | backend | 心跳（200ms/次） | 输出 |
|---|---|---|---|---|
| `tests/data/real-big.xlsx`（50,001 行 / 773 KB） | **31 ms** | `xlsx-self` | 未到 200ms | 1001 行（含截断提示） |
| 用户切件（436,000 行 / 47.41 MB） | **116,772 ms ≈ 117 s** | **`read-excel-file`** | **0 次（期望 584 次）** | **仍只 1001 行**（提示「已读取 436000 行，每 sheet 保留前 1000 行」） |

### 因果链（一句话）
**47.41 MB 过了 50MB 护栏 → 但 `sharedStrings` 87.4 MB ≫ 4 MB 护栏 → 按 t33 设计回退 `read-excel-file` → 该库在主线程啃完整份工作簿（117 s，页面全程无响应）→ 输出仍受 1000 行上限 ⇒ 2 分钟只换来 1000 行。**

### 结论与候选修法（**均需拍板**；观察期内不改行为，已登记 HANDOFF §3 **A9**）
1. **预检 + 知情同意**（最小改动）：解析前只读 ZIP 中央目录估 `sharedStrings`/`sheet` 解压尺寸 → 超阈值先提示「约需 N 分钟且页面会暂时无响应，是否继续」。我们**已有**中央目录读取能力（`findEocd` / `zipEntry`）。
2. **回退路径加止损**：库路径同样只取前 N 行即停。
3. **流式 + Worker**（A7.3-#9，治本）：不冻 UI + 进度 + 可取消。

### 真机 UI 复测（用户本人跑，2026-09-17 15:44，同一份 47.41 MB 切件）
- **产物 = `…\Downloads\WizTree_20260917150849_cut.md`：351,914 B / 1003 行 / 1001 表格行** —— 与我侧 headless 产物的**字节数完全一致**（351,914 B）⇒ 同一输入同一产物，**结果可复现**（仅比字节数与首尾内容，未逐位比哈希）。
- **耗时 59,402 ms**（用户读到的 UI 显示值）vs 我侧 headless **116,772 ms** ⇒ **2× 差距，未解释**：用户为前台桌面真浏览器，我侧为 headless msedge + 沙箱调度。**标注为待验证**（要钉死需同机对照复测）。
- **浏览器弹出「页面无响应（停止运行 / 等待）」**，用户选「继续等待」后拿到结果 ⇒ **外部独立确认**了心跳探针的结论（转换期间主线程被完全占住）。
- **关键提醒**：该文件 59 秒只换到 **1000 行**（436,000 行的 0.23%，受 `XLSX_ROW_LIMIT` 限制）；且**桌面耗时 ≠ 手机耗时**（目标平台是手机，同文件在移动端大概率分钟级甚至崩标签页 —— 这正是 50MB 护栏的存在理由）。
- 附带发现（用户文件侧）：其切件保留了 WizTree **标语行**（第 1 行）→ 成为 md 的表头行；`.tmp/cut-csv.mjs` 可正确丢弃标语行。

### 方法论沉淀
- **「护栏内」≠「体验可接受」**：50MB 只是粗门；真正的瓶颈是**解压后的结构与规模**（strings 尺寸 / 行数）。任何「按文件字节数设的护栏」都要配一条「按解压后规模/结构」的二级判据。
- **心跳探针是可复用的取证手段**：`setInterval(200ms)` + 转换后读计数，一眼区分「慢」与「卡死主线程」——本次 0 / 584 就是硬证据。
- **对照实验必须有**：同一次运行里放一个 `real-big.xlsx`（31 ms）作参照，才能断定 117 s 是「大文件固有」还是「路径选择问题」（实测是后者：两条路径 4000× 差距）。

## 2026-09-17 本批：截断提示写进产出（B①）+ 同构件多页样例（D）+ README 口径（A）

### B①｜截断提示写进产出（`130a927` 先红 → `f6878e7` 实现）
- **动因**（手机侧整合报告 §3.7 真机实测）：3,054 行输入 → 恰好 1,000 行产出且**零标记** ⇒ 用户下载/转发的 `.md` 与完整产出外观无差别。用户拍板**形态①**。
- **口径**：**仅 `meta.truncated` 时**于 markdown **首行**插**单行 HTML 注释**（渲染不可见、原文可 grep），内容**取转换器的截断原文**（口径单一来源）；**未截断件不得出现**（防误标）。
- **实现点（单点）**：`src/convert.js` 的 `runConverter` 收口 —— 不在各转换器里各写一遍。
- **契约**：组 G2 续号 **L4c**（首行注释 + 含「每 sheet 保留前 1000 行」+ 单行收尾 + 后接空行）· **L4c-2**（反向守卫：`sample.xlsx` 未截断产出不得含注释）。三态：先红（L4c ❌ / L4c-2 ✅）→ 后绿（7/7）。
- 产物 **121,844 B / `9E30306B…`**；契约 **262/260/0/2**。

### D｜同构件多页样例 `sample-scale-multipage.pdf` + 契约 U10（`6a2c733`）
- **动因**：手机侧建议把**野生素材**（真机 WPS 9 页通知）入库当第 20 个对照样例 —— **被我方驳回**：那是真实公文（真实单位名 + 文号 + 个人路径），撞隐私红线与 `tests/data` 公开性。**替代方案 = 合成同构件**（用户拍板 D）。
- **形态**：3 页 · 逐页 `Tm` 缩放 **0.05 / 0.045 / 0.03**（照真机实测区间）· 逐字 `TD`（dx=280=1em）· 每页**首行两个文本对象**（对象 B 的 x = 对象 A 名义末尾）· 虚构文字 · 确定性字节（1,960 B / `C5A00DBE…`）。
- **契约 U10**：去空白后按 `NA NB PC PD → QA QB QC QD → RA RB RC RD` 顺序单调 + 3 个页标记齐备。
- **负对照**（.tmp/mk-pdf-neg.mjs 去掉 `tdx` 缩放 → 重建 → 组 U）：**U9 与 U10 同时红**（fail 3），还原后 12/12 ✅ ⇒ 两条都真在守 P1 家族。
- ⚠️ **本批踩的坑（值得记）**：**首版夹具把行序放反了** —— 我按"y 越大越靠上"把双对象对放在 `y=780`，而翻转空间（`1 0 0 -1 0 842 cm`）里**阅读顺序是 y 递增**（`sample-scale-td.pdf` 就是 720→750→780）。U10 首跑红，诊断输出 `<!--page1/3-->PDPCNANB…` 一眼看出**产品是对的、夹具错了**。**教训：写夹具前先对齐既有夹具的坐标语义；红了先怀疑夹具。**

### A｜README 第三类限制（聊天 App 内置 WebView）
- 补：微信内置**只有相册** · 钉钉内置**「上传文件」灰** + 点下载**可能无反应** · **QQ 内置正常**（对照）⇒ 给 **「⋯ → 在浏览器打开」** 引导；出处 = 手机侧真机矩阵（截图由我方**目视复核**确认「上传文件」禁用）。

## 2026-09-17 晚 · 手机侧第二包（ADB 内存取证）核验：峰值 12.63 GiB / 文件 35.86 MB = 369×

### 交件与核验
- `doc2md-mobile-20260917 (1).zip`（**2,157,540 B / `0A8778C1…`**，23 条目）→ `.私档/验证/手机实测-20260917-第2包/`；**13 个既有物证逐字节不变**（含 P1 三端一致的两份产物、大文件矩阵三份产出、3 张截图），变更 3 份文档 + 新增 ADB 目录（3 文件）。
- **我方按时间戳分组独立核算**（不采信转述；150 时间点 / 20:59:45→21:09:18，"卡住那轮"）：

| 指标 | 我方核算 | 交件 |
|---|---|---|
| 峰值 PSS+swap（全部 quark 进程） | **12.63 GiB（13.24 GB）** @21:08:33 | 11.24 GB |
| 峰值 PSS（不含 swap） | **11.58 GB** | — |
| 渲染进程峰值 PSS | **10.16 GiB** | — |
| 平台期 total / renderer | 8.69 GiB / **6.29 GiB**（21:02→21:07 不动） | 渲染 6.5 GB |
| 倍数 | **369×**（÷35.86 MB）· **34.6×**（÷解压 382 MB） | 321× / 31× |

⇒ **交件口径保守，我方数字更大**（其值更接近"仅 PSS"档）→ 结论更强。

### 归因边界（本轮最重要的一条方法论）
交件断言「渲染进程**自身分配失败**、非系统回收」，依据是「有 `am_proc_died`、无 `am_kill`」。**该推理有洞**：`am_kill` 只记 **AMS 发起**的回收；**内核 LMK/OOM-kill 不写 `am_kill`**，而交件自述 **`dmesg` 被 SELinux 挡**、logcat 为**按标签过滤** ⇒ **两种假设都会留下 `am_proc_died`**，当前证据**不足以**在二者间定论。
**低成本补证（下一轮）**：① `adb logcat -b all -v threadtime` **不过滤**（LMK 行常在 `lowmemorykiller`/`lmkd` 标签下出现）；② `adb shell dumpsys dropbox --print`（常含 `data_app_crash`，无需 root）；③ Chromium 崩溃页文本（「Aw, Snap」/「页面崩溃」）一眼区分"自杀"与"被杀"。

### 我们这边的两条方法论沉淀
- **单位口径要先对齐再比数**：交件的 "MB" 在不同处混用 **MiB/MB**（如 361.0 MiB = 378.6 MB），我方核算一律标明两种单位，避免把单位差误判为分歧。
- **"无 X 日志" ≠ "X 没发生"**：先确认该事件的**日志通路**（AMS vs 内核 vs 崩溃上报）是否在采集范围内 —— 本轮就是「过滤后的 logcat + 被 SELinux 挡住的 dmesg」两条通路同时缺失。

### 交件内部不一致（已列入待澄清）
README §0.2 称同一份 47.4 MB 文件曾「**43 秒转完**」，而 §1 备注「47.4MB 那档**真机上从未产出成功**」—— 二者需澄清；若确有成功轮次，请附产物/截图（将是**首次**手机端转完 `3-big` 的证据）。

### 对主线的直接影响
- backlog §2.5 的**共同前置「先量峰值内存」✅ 完成**（数据即此）；
- **#8（流式 / 解析到够 1000 行即停）拿到量化目标**：峰值 **12.6 GiB → 目标 MB 级**；
- 「同一文件三种命运（43 s 完成 / 20 s 崩 / 磨 20 min 无果）」⇒ **天花板随可用内存浮动**，**印证不必追精确阈值**；
- 与 **A9/A10**（预检 + 进度 + 超时）方向一致 —— 手机侧结论与我方独立收敛到同一处：**降低单次分配量**。

6. ⭐⭐ **手机侧「无线调试 / adb 配对」会把 PC 侧的 USB adb 挤成半死态**（本轮真凶；**用户自己提出假设，A/B 一次证实**）：
   - **现象**：PC 侧 `adb devices` **列得出**设备（`device` 状态），但 `adb install`（卡在 `Performing Push Install`）、`adb push`（**连 7 字节都卡**）、`adb shell` **全部无限挂起**；期间还出现过一次 `offline` 和一次 `- waiting for device -`。
   - **根因**：**手机侧那个 DSH 开着「无线调试 / adb 配对」**（它自己也有 adb 工作要做）⇒ 设备侧 `adbd` 同时持有 TCP 传输，PC 的 USB 传输被挤成**半死**：**控制通道还能应答**（所以设备列得出），**数据通道完全不通**。
   - **判据（A/B 定因果）**：用户**关掉手机侧无线调试** ⇒ 同一条 **7 字节** `adb push` **立刻成功**（`1 file pushed, 0 skipped. 0.0 MB/s (7 bytes in 0.005s)`）；关之前同一条命令挂死。
   - **防再犯**：① **用 USB adb 做真机取证前，先确认手机「开发者选项 → 无线调试」是关的** —— 手机上也跑 agent/工具链时，它会自行开配对；② 遇到「**列得出、动不了**」这个组合，**第一件事查无线调试开关**，别先怀疑线材/驱动；③ **7 字节 push 是最便宜的探针** —— 它能一眼区分「控制通道坏」与「数据通道坏」（本轮靠它把诊断从"全坏"修正为"半死"）。
   - ⚠️ **如实登记我的两次误判**：先判成「线材/带宽问题」，又判成「Chrome 抢 adb」——**都错了**；真凶是用户提出的第三条假设（手机侧 adb 配对）。教训：**「列得出、动不了」这个指纹本身就指向"设备侧多了一条传输"，不该先往外围（线/浏览器）猜**。

7. **adb 数据通道的三种表现形态（同一脆弱面）+ 一条待验证的诱因**：
   - **形态 A**：`adb devices` 显示 **`device`**，但 `install` / `push` / `shell` **全部无限挂起**（连 **7 字节**都过不去）—— **已确证诱因 = 手机侧「无线调试 / adb 配对」开着**（关掉即恢复，A/B 定因果）。
   - **形态 B**：**`offline`**（`kill-server` 后新起的 server 仍握不上手）。**用户确认此时无线调试并未开启** ⇒ 候选诱因 = ① 浏览器 DevTools 的发现机制（Edge/Chrome 会自己拉一个 adb）占着设备 ② 设备侧 `adbd` 卡死 ③ 线/口。**三者均未验证**（2026-09-20 21:00 止损，用户决定到此为止）。
   - **形态 C**：`- waiting for device -`（`adb logcat` 侧出现过一次）。
   - **防再犯（已可执行的部分）**：① **USB adb 取证前先确认手机「无线调试」是关的**；② 见到「**列得出、动不了**」这个指纹，**先查那个开关**，别先怀疑线材/驱动（我先后误判为"线材/带宽"与"Chrome 抢 adb"，都错）；③ **7 字节 `push` 是最便宜的判据**（一眼区分"控制通道坏"与"数据通道坏"）；④ **取证脚本一律给 adb 调用加超时** —— 否则"卡"会吞掉整轮时间（本晚两次）。
...
**判定方法（可复用）：先翻"这个仓对这个东西有没有既定口径"，再看这一份的具体内容。**
- 仓里**早就写死过**这条口径（2026-09-16 用户拍板路线 A）：「工具链 PR **先本地实测再落提交**；**不逐个 merge** → 本地一批 + 一次 push，PR 由 bot 巡检自动关」⇒ **不需要重新论证"要不要合"，那是翻案**。
- 但**只有口径不够** —— 还得**把这一份拆开看**，否则等于拿规则当结论。

**坑 / 根因 / 防再犯**：
1. ⭐⭐ **"CI 绿"是一个关于**某棵树**的陈述，不是一个关于**你的树**的陈述**：PR #8 的绿是在 `062cc04` 上拿的，**不含当时还没推的卡 012 三个新 `tools/*.mjs`** ⇒ 先合 PR、再推新代码，**绿可能变红，而且你分不清是谁的锅**（sonarjs 4.2.1 若加规则，正好打在新文件上才是大概率）。**防再犯**：合并类决策先问一句「**它的绿覆盖到我要推的东西了吗**」；覆盖不到就**先推自己的、再让 bot 更新分支、再看**。
2. ⭐ **bot PR 会**换源**：Dependabot 在 GitHub 上跑 ⇒ 用 `registry.npmjs.org`；而本机 lock 里**大量条目是 `registry.npmmirror.com`**（2026-09-16 实测 176 处）。⇒ 它的 diff 会把**无关条目也换掉**（改动面远大于实际升级）。**本地方案**：保留既有解析地址、**只对新/变更包写 npmjs**。
3. ⚠️ **"绿的 PR 也踩过许可门"**：`docs/licenses.md` **L41** 写着**禁止 LGPL**，而工具链的 **`eslint-plugin-sonarjs` 自己就是 `LGPL-3.0-only`**（已在仓、且配置启用了它的规则 ⇒ **是活的**）。PR #8 又带进一个同许可的 `@sonarsource/analyzer-commons-configurations`（版本号还是**预发布形态** `2.31.0-5284`）。⇒ **这不是 PR 的锅，是"门"和"现状"早就打架**（devDependency 是否属于"引入本项目"，口径没定义）。**防再犯**：许可门这类"第一道门槛"式的**绝对表述**，要么定义清楚适用范围（交付面？含 dev？），要么就别写成绝对句 —— 否则**每次依赖变动都会撞它一次，而每次都靠人临时判断**。
4. ⚠️ **引历史教训要引准**：上次 **PR #7（eslint 9→10）的 CI 本身是 failed**（当时记录："直接 merge 必然红"）—— 那是「**bot PR 会红**」的证据，**不是**「CI 绿也会炸」的证据。⚠️ **把这两句混起来用，就是用一条不成立的证据去支持一个成立的结论**（结论对、证据错 = 下次结论就不可信了）。
5. ⚠️ **顺手发现一处"lock 与 package.json 不同步"**：`package-lock.json` 根条目**没有 `engines`**，而 `package.json` **有**（卡 009 加的 `"engines": {"node": ">=24"}`）⇒ `npm ci` 不校验它（所以一直没红），但**任何 `npm install` 都会重写 lock**（凭空多一个 diff）。**防再犯**：改 `package.json` 的元数据字段（`engines` / `license` / `name`）时，**同时跑一次 install 让 lock 跟上**，或者明知故留并登记。
6. ⚠️ **我自己在现场又踩了一次本仓写死的坑**：`git show HEAD:package-lock.json | Select-String …` ⇒ `程序'git.exe'运行失败： 拒绝访问`（native 命令进 PS 管道）。**正确做法 = 读文件用 `read` 工具**（工作树文件与 HEAD 一致时根本不需要 `git show`）。**这条本仓已记过至少 3 次，我又踩了第 4 次** ⇒ 记在这里不是给纪律添砖，是提醒**"知道"与"手不犯"之间还有距离**。
...
**结论**：反馈载体从「**文档流顶部 13px 灰字**」换成「**`position:fixed` 底部浮层**」（Material Snackbar 形态 · 15px · 自动消失 6000ms），**只加在原生分支**（Web 侧浏览器自带下载完成反馈，再加一层 = 用户明确否掉的"两个成功反馈"）；**Web 侧一字未动**。契约组 **Z4** 共 6 条；产物 136,560 → **137,866 B**；真机复现 + 真实触摸 + 截图三重取证（`.私档/验证/卡011-真机实测/a10-toast.png`）。

**坑 / 根因 / 防再犯**：
1. ⭐⭐ **相对词判据会被样本绕过（本轮真正的根因）**：`A8` 写的是「**可见**成功提示」—— **"可见"没定义相对谁**；而第二轮真机验收恰好取**单文件**（页面短到不用滚）⇒ **条件绕过了缺陷**，于是"**三面全绿**"与"**用户看不见**"**同时为真**。**防再犯**：凡「可见 / 明显 / 及时 / 清楚」这类**相对词**，写判据时**必须连参照系一起写**（本轮 = `window.innerHeight`），并且**把现场条件也写成断言** —— 本轮 **Z4-0**（3 卡 + `scrollY>0` + `#status` 已在视口外）在**先红阶段就是绿的**，它的作用**不是判缺陷，而是判"测试到底有没有复现用户场景"**；现场不成立时，主判据会退化成恒真。
2. ⭐ **`display:none` 的元素 `getBoundingClientRect()` 全 0 ⇒ 只判"落在视口内"必然恒真**：本轮"可见"判据做成**三层**（存在 → `display≠none` → **rect 非零**）。**防再犯**：写几何断言前先问「**隐藏态下这条还成立吗**」。
3. ⚠️ **自动消失的 UI 会与取证抢跑**：浮层 `TOAST_MS=6000`，而「node 退出 → PowerShell 起 adb → `screencap`」实测 ≈ **5.5s** ⇒ **第一次真机截图拍空了**（图里没有浮层，可 CDP 读回明明写着 `fullyInViewport: true`）。**防再犯**：① **取证贴住被测动作**（本轮改成在**同一个 node 进程里**、触摸后 700ms 内 `screencap` + `pull`）；② **截图与 DOM 读数必须同时留** —— 只有 DOM 会漏"真渲染出来没有"，只有截图会漏"数值合不合规"。
4. ⭐ **"我这边的桩"证伪不了"我这边的假设"**：本轮顺带更正的那句假注释（「插件回报**真实**文件名（重名会改名成 `x.md (1)`）」）**在 CI 桩下永远绿** —— 因为桩是照我的假设写的。**防再犯**：跨语言 / 跨进程的**行为假设**（JS 假设 Java 会回什么）**必须读对方实现**，并**在真环境验一次**。
5. ⚠️ **共享模板不要塞单端样式**：浮层样式若写进 `src/template.html`，等于**改了 Web 的样式表**（那是两端共享模板），与"Web 侧一字不动"直接冲突。**防再犯**：**只在单端出现的 UI，元素与样式一起在单端代码里懒创建** —— 差异也才留在 `src/` 里可读（与卡 010「一处真相」同口径）。
6. ⭐ **`B3` 断言过户的第一份实跑**：逐条判「仍成立 / 已过时 / 已静默 / 已死」时，**光看"绿不绿"没有信息量** —— 要点是问「**这条能不能红**」。本轮据此查出 **Z2-2 的 `assert.equal(dl, null)` 在失败时会格式化 playwright 对象图 ⇒ 把整个测试进程 OOM**（第二轮实测过），处置 = 改写法、**判据一字未变**。**局限也写清楚**：**没做逐条故障注入**，"仍成立"是"实跑 + 各自先红记录 + 人工审读"三重支持下的判断，不是注入验证过的。
7. ⭐⭐ **"真机验过了"必须点名验的是哪一面（被用户当场问出来）**：我第一轮把**成功面**在真机上跑通就写了"真机实测 ✅"，**失败面只跑了 CI 桩** —— 用户一句「**话说下载失败这个试了吗**」把它戳破。补测后三面皆绿（成功 / 写入被拒 / 无插件，均有截图；**zip 分支未在真机验**，已如实登记）。⚠️ **这不是新错，是同一个错的第三次**：① 第二轮拿**单文件**当"可见"的样本；② 用**桩**证明"插件回报真实文件名"；③ 用**成功面**代指"真机验过了" —— **共同形态 = 用一个更省事的样本/层，代替用户真实会走的那条路，然后宣布绿**。**防再犯（固化成写法）**：**"真机 ✅"要写成一张面清单**（成功 / 失败 / 各成因分支，**逐格标 已验 / 未验**），**未验的格子必须写进回执**，**不许用一句"真机验过"盖住整张表**。
...
**根因**：① 该标题是当时随手写的**总括句**，四条各自的真实载体**从未逐条 grep 核验**；子条目的更正是**事后补的**，只修了那一条，**没回头修父标题**。② 更一般的形态 = **「整包声明」里混装了不同载体的东西** —— 只要有一条不成立，整句就是假的，而它看起来像"汇总/权威"。

**实测（可复算）**：项目 `AGENTS.md` grep `私档|隐私红线|noreply|no-pager|nutc|童子军|重构配额` ⇒ **L15（红线 9）/ L60（重构配额）/ L81（`--no-pager`）**；全局 `~/.dsh/AGENTS.md` 同模式 ⇒ 仅 **L37 / L49 / L50 / L64**（全是隐私条），`童子军|重构配额|no-pager|nutc` **零命中**。⇒ 四条里**只有「全局第 11 条」一条真在全局文件**。

**处置**：标题改为**逐条标注真实载体**（项目 L15 / 项目 L60 / 全局 L37 / 项目 L81）+ 原句**作废留痕**（不静默删）+ 附核验命令，全部落在 HANDOFF 原位。
...
## 2026-09-20 · 卡 011（执行线）：保存环节不再静默 —— 原生壳里的「做不到」必须明说

> 完整回执见 `docs/任务台账.md` 卡 011 节。本节只留**一手坑与结论**。

**结论**：新增 `isNativeRuntime()`（`Capacitor.getPlatform() !== 'web'`）+ 文案表 `NATIVE_LIMIT`；**原生壳 + 无插件** ⇒ 状态栏「保存失败：本机未加载保存插件，文件未保存」**且不回落**（回落 = 静默）；**原生壳点 zip** ⇒「暂不支持 zip…请改用「🖼 下载 .md（图片内嵌）」」。**Web 侧零变化**。契约组 Z3（+3）**先红 `3/0/3`** → **后绿全量 `305/303/0/2`**；产物 135,620 → **136,391 B**。

**坑 / 根因 / 防再犯**：
1. ⭐ **契约数变了必须与 `BASELINE.json` 同提交**（+3 ⇒ `305/303/0/2` 与 `302/299/0/3`）—— 否则 CI 的 TAP 对账步会红，而那是**门禁**、不是噪音。
2. ⚠️ **测试进程 OOM ≠ 代码错**：先判**环境**（本机可用内存实测 6.7 GB，被 1.2 GB 的 node + 多个 msedge + WSL + Gradle daemon 占着）⇒ 放宽堆（`--max-old-space-size`）+ **别在断言里 inspect 大对象**（`assert.equal(<playwright Download>, null)` 失败时会格式化巨型对象图）。**判定条件一字未动**，只换表达方式。
3. ⭐ **「不静默」要覆盖"我以为不可能发生"的分支**：本卡两处静默都在**降级路径**上（无插件回落 / 未接线的 zip）—— **降级可以，静默不行**；写降级分支时**必须同时写它的可见反馈**。
4. ⚠️ **边界要写进回执**：「**不再静默**」≠「**已修好**」；根因（那台机器上插件为何没注册）另立，**需那台设备 `logcat`**。
...
8. ⭐⭐ **异机抽查抓到我自己实现的两个"静默"洞**（用户借 vivo NEO7 / Android 14 实测：同包、转换成功、**点下载后状态栏一字未变、`Download/` 无文件**）：
   - **洞 A：探测不到插件时"静默回落"** —— 我的 `saveTextArtifact` 在 `nativeSavePlugin()` 返回 null 时**悄悄走 `<a download>`**；而在 Capacitor WebView 里那条路**本来就无效**（卡 008 的 A4）⇒ **点了没反应且无任何提示**。**正确姿势：原生平台（`Capacitor.getPlatform() !== 'web'`）却探不到插件 ⇒ 显式报错**，别把"降级"做成"静默消失"。
   - **洞 B：主按钮没接线** —— 带图文档时**主按钮是 zip**（`📦 下载 .md + 图片（zip）`），而 zip 路径本卡未接线 ⇒ 对**带图文档**（用户真实主力场景）"装得上、存不了"依旧。**教训：接线要问"用户最先点哪一颗"，不是"我接了哪一颗"。**
   - **归因方法论**：状态栏文案是**第一判别器** —— **"报错文本没出现"本身就否掉了一整类原因**（说明失败发生在**我加的错误处理之外**）。⇒ 以后加"不静默"处理时，**每个静默分支都要能被一句文案区分开**。
   - **两个"看起来像"的机制要先排除再下结论**（本次都排除了，省下误诊）：① **SW 缓存老页面**？查 `sw.js`：**导航是网络优先** ⇒ 覆盖安装必拿新壳（cache-first 只对资源）；② **点错按钮**？看卡片实际按钮（无图片的文档没有 zip 那颗）。
   - ⭐ **既有自测钩子能当远程探针**：插件 `load()` 首次运行会写 `doc2md-native-selftest.md`（18 B）到 `Download/` ⇒ **"那台机器上有没有这个文件"= 插件到底加载没加载**，不用插线就能判。**遗留的临时钩子有时候是唯一的远程诊断入口** —— 拆它之前先想清楚代价。
...
### 做了什么
1. **项目 `AGENTS.md` L60 改写**（`649c4ce`）：核心句换成「**必须做，或必须显式说明为什么不做**」；原措辞「每次改动代码顺手做『一点点』重构」**明确作废**（理由写在行内：它允许"不提就静默跳过"）；**"做"的判定口径全部沿用**（单次 ≤50 行 / 只抽函数·表驱动化·消灭复杂度警告 / 减脂批配等价性台 / 断言全绿·lint 零新增 / 修 bug 与重构分提交 / metrics before → after）；新增**"不做"的唯一合法形式 = 显式说明**（回执 / 提交信息 / 报告里写理由）。**禁的是"静默跳过"，不是"一定得改"** —— 判据 = **有没有那句话**。
2. **`docs/HANDOFF-主开发线.md` L67 同步**：该行是旧措辞的**活副本**（`- 重构配额（童子军规则）：每次改动顺手 ≤50 行重构…`）⇒ 改为**指向 `AGENTS.md` 的指针** + 新标题句（细则不再复述，消灭漂移面）。
3. **全局 `~/.dsh/AGENTS.md`：改不了 —— 如实上报**（工作区外 + 沙箱拒绝写入 + 一次性升权被用户拒绝 ⇒ **不绕行、不找旁路**；按协作铁律 2/4）；**用户 2026-09-20 后续拍板：全局不新增此条，本项收官** ⇒ 规则的跨会话载体 = 项目 `AGENTS.md` + **mneme 记忆条目**。
4. **全仓同义措辞 grep 逐条分类**（`童子军|重构配额` 命中 **15 处**）：**1 处权威**（`AGENTS.md` L60）+ **1 处活副本**（`HANDOFF` L67，已改）+ **13 处历史记录**（`HANDOFF` L8 / L668 / L853、`DEV-NOTES` L591 / L726 / L1518、`任务台账` L835、`CONTRACT.md` L762 / L877 / L917 / L923 …）—— **历史记录一律不回改**（改 = 伪造当时的口径）。
5. **本批"重构"声明**（依新规必须显式说明）：只动文档（`AGENTS.md` + `docs/**`），**无生产代码可重构** ⇒ 不做，理由即此句。

### 坑（现象 → 根因 → 防再犯）
1. ⭐ **「全局 AGENTS.md 里有这条」是一句从未核验过的声明**：用户问「全局里面没有重构的内容？」，实测**全局文件 9 组模式**（童子军 / 重构 / 配额 / refactor / ≤50 / 抽函数 / 表驱动化 / technical debt / 技术债）**全部零命中**；该文件只有 7 个一级章节（全局工具偏好 · 记忆纪律 · 协作铁律十一条 · Privacy & Copyright Review · 排障纪律 · 主 SOP 骨架 · 安全基线）。
   - **根因**：`HANDOFF` L65 表头写的是「**纪律沉淀（已写进 AGENTS.md / 全局 AGENTS.md）**」—— 把"**应该**写进"当成"**已经**写进"，**声明式回执没有判据命令**；用户与后续会话都据此以为全局有这条。
   - **防再犯**：① 纪律/规则**唯一权威源 = `AGENTS.md`**，其它文档**只写指针不复述细则**（复述即漂移面）；② 凡落盘「已写进 <文件>」，**当场跑判据**（grep / `Select-String`）并把**命中行号**写进回执 —— **声明必须能被一条命令复核**。
2. **改写规则只改主文件、没扫副本**：`AGENTS.md` 改完后，`HANDOFF` L67 仍以旧软措辞**活在权威文档里**（照旧措辞理解 = 这次改动等于没生效）。
   - **防再犯**：改写/作废任一纪律时，**先 grep 全仓同义措辞**，逐条标「权威 / 活副本 / 历史记录」三类**再动手**（本次即此法）。
3. **"用户说全局也改" ≠ "全局真的有内容可改"**：核实结果与用户预期不符时，**直接说"没有"并给证据**，不要为了顺着预期去"先造一处、再改掉它"。（后续拍板：**全局不新增此条**。）
4. ⭐ **同一条规则有 4 个存放点 —— 改写时四处立刻打架**：① 项目 `AGENTS.md`（权威）· ② `HANDOFF` L67（活副本）· ③ **`mneme` 跨会话记忆条目**（`constraint`，id `53c9b2a6-eb40-4d13-9569-7e40c45e88fb`，**每次会话自动注入** —— 效果上"最像全局配置"的其实是它）· ④ 旧会话里的口头口径。**记忆条目是注入式副本：不跑判据、不改就静默过期**，本次差点漏（用户先问「全局里面没有重构的内容？」，第一轮只查了 AGENTS.md 文件）。
   - **处置（用户 2026-09-20 拍板）**：**记忆条目已更新**为新口径（标题改「重构纪律（童子军规则）：必须做，或必须显式说明为什么不做」；沿革注明原措辞作废 + "不做"须显式说明 + 全局不含此条）；**全局 `~/.dsh/AGENTS.md` 不新增**（用户原话「全局就不加了」）。
   - **防再犯**：改写纪律时**四处一起列**（权威 / 文档副本 / 记忆条目 / 口头），**记忆条目按"注入式副本"单独核对一次** —— 它不在 git 里，`grep` 查不到。
5. **中文提交信息里夹 ASCII 双引号 ⇒ `-m "…"` 被 PS 提前闭合（长得像 git 的错）**：本批第二次提交实测报 `error: pathspec '显式说明义务）。' did not match any file(s) known to git` —— 原因是我在 `-m "…（含沿革与"显式说明"义务）…"` 里用了 **ASCII 双引号**，PS 在此处**闭合字符串**，余下片段被当成 pathspec 交给 git。
   - **判据**：报错说"某条中文字符串不是 git 认识的文件" ⇒ **先怀疑 shell 引号，不要怀疑 git**；且**提交没发生**（文件仍处 staged，可原样重提 —— 本批即如此恢复）。
   - **防再犯**：提交信息里的强调一律用 **「」/『』**，**禁用 ASCII 双引号**（或整段走 here-string `$m = @'…'@` 再 `-m $m`）；同类第 2 次出现即把 here-string 写成固定用法。

## 2026-09-20 · 卡 007 回箱核验（Linux 弱独立）：**判据升级到 tree 级**，但我的断言文案误导了方向

> 回箱 `.私档/linux/20260920-数值长尾显示归一/`（报告 16,865 B + 回执 + SHA256SUMS + 15 份原始日志 + 5 份自写脚本）。**核验结论：通过** —— 哈希 **26/26 逐条一致**；关键数字全部从**原始日志**复现（组 G9 `7/7`；契约 `297/295/0/2` 与 `294/291/0/3`；对账 EXIT=0；**两注入 A `pass 3/fail 4` · B `pass 2/fail 5`** = 与出单方预跑值逐项一致；**A5 新值四档** `61990/23611/352023/352027`；**A5 交叉验证（注入 A ⇒ 旧值）** `66140/23611/352755/352759` 全中；metrics 21·527·超限 0·0.4%；产物 `135,398 B / CD7977E4…1507`；`porcelain = 0`）。

### ✅ 兑现（连续第三轮）
1. **预跑实测负对照 ⇒ 零偏差复现**（卡 005 靠对方纠错 → 卡 006/007 连续两轮预期与实测一致）。
2. ⭐ **判据升级链条走通**：卡 005/006 用 **blob 级**（提交未推送）→ 卡 007 用 **tree 级**（已推送：`HEAD^{tree} == 51b2787^{tree}`、补丁树 == `9ce1d68^{tree}`）⇒ 当初为绕开"未推送"而定的 blob 判据**自然升级**，不是权宜之计。
3. ⭐ **"交叉验证"是我主动加的、并被采纳为最强证据**：单看"新值 = 我算出来的"只能证明**一致性**；**注入 A 后四档逐字节回到旧值**才能证明**因果**（产物变化的唯一来源 = 这条显示归一）。**⇒ 出单时值得固定加一条"反向回到旧值"的要求**（本轮已验证其可行与价值）。

### 坑（现象 → 根因 → 防再犯）
1. ⭐ **复合断言的报错文案必须覆盖"全部失败模式"，否则它会把排查者指向反方向**：
   - **现象**：G9-3 一条断言同时管两件事（`布尔/文本格原样` + `数值格归一为 34.2`）。注入 A（关掉归一）后它确实变红——**红得对**；但文案写的是「**非数值被误改**」，而 actual/expected 显示 `true`/文本格**根本没被动**、真正的问题是数值格没归一。Linux 侧据此登记：「**断言文案与注入失败模式不匹配**」并**明确不改断言**（对）。
   - **根因**：**一条断言里放了两个正交条件**时，我把报错文案写成了"只覆盖其中一个失败模式"的句子 ⇒ 文案成了误导源（而不仅仅是信息不足）。
   - **防再犯**：① 复合断言的文案要**逐条列出它覆盖的失败模式**（本次改为明说两种并提示按 actual/expected 逐格判读）；② 更好的做法是**拆成两条断言**（各自文案就能精确）——本次为遵守"断言定稿后不改结构"未拆，**登记为下次同类断言的写法偏好**（新写的复合断言直接拆）；③ **归因要看 actual/expected，别只看 error 首句**。
2. **两处统计口径不同时，要把差异来源写清楚**（本次做到了）：他们独立台统计"长尾形态"得 `265/0/32/32`（**产物全文**），我的必答题 1 是 `266/0/3039/3039`（**窗口内 XML**）—— 差异来源 = 日期样式列在产物里已成日期 + 产物只含前 1000 行。**⇒ 他们主动标为"方向性旁证"、我复核认可**：**口径不同的两个数并排出现时，必须写清"为什么不同"，否则下一个人会以为是矛盾**。

## 2026-09-20 · 卡 007（执行线）：xlsx 数值长尾的显示归一（渲染层）

> 完整回执（A1–A10 逐条 · **两道必答题的实测** · 边界用例表 · 先红/后绿原始输出 · A5 重冻逐档说明 · 第 7 条对表）见 `docs/任务台账.md` **卡 007** 节；本节只留**一手坑与结论**。

### 交付（未推送 —— 发布动作人执）
- **落点 = `src/xlsx.js` 的「值 → 文本」那一步**（`serialDateOrRaw`）+ 新增 `sigDigitsOf` / `formatNumericLiteral`。**解析阶段 10 个函数逐字未动**（`git worktree` 切片比对）。产物 **135,398 B / `CD7977E4…1507`**。
- **先红 → 后绿**：契约组 **G9**（7 条）：先红 `tests 7 / pass 3 / fail 4` → 后绿 `7/7`、全量 **297/295/0/2**（带夹具）与 **294/291/0/3**（干净检出）。
- ⭐ **两道必答题实测**：① 四档**有**长尾（266 / 0 / 3039 / 3039）⇒ A5 走「新值 + 逐档说明」；② **「50 行」= 两档大档 R 列的 50 个长尾单元格**（首例 `R38 = 34.200000000000003`）—— 与手机侧观察逐字对上。
- **A5 重冻**：`1-small` 66,140→**61,990** · `2-mid` **一字不变**（干净对照）· 两档大档各 −732（R 列那 50 个）。**夹具零改动**。

### 坑（现象 → 根因 → 防再犯）
1. ⭐ **`sonarjs/super-linear-regex` 连拦两版 —— 首尾裁剪别用正则**：
   - **现象**：首版 `sigDigitsOf` 用 `/^(\d*)(?:\.(\d*))?(?:[eE][+-]?\d+)?$/`（**嵌套量词**）⇒ `lint` **error**；改成"先切指数、再手写扫描数字"后，**又**被拦在 `.replace(/0+$/, '')`（**开头不锚定 + 结尾锚定**：失败时逐位重试 ⇒ O(n²)）⇒ 最终**首尾零裁剪也改成手写指针**（`while (a < b && digits[a] === '0') a++`），lint **0/0**。
   - **根因**：该规则不只是"嵌套量词"——**任何"开头可滑动 + 结尾锚定"的量词**（`x+$`）在最坏情况下都是超线性的；而 `.replace` 的旧习惯正好常写这种。
   - **防再犯**：**字符串首尾裁剪一律手写指针**（O(n)、零正则风险）；要写正则就**两端都锚定**（`^x+$` 也要小心，能用字符类 + 循环就别用 `+$`）。⚠️ 本轮**两次**都栽在同一规则上 ⇒ 已够"同类 ≥2 次"，可作沉淀候选。
2. ⭐ **断言把"对照格"一起扫了 ⇒ 因错误原因变红（差点误导实现）**：
   - **现象**：G9-4 首版对**整行**断言「不含 `46282`」，目的是"日期样式那格不许吐序列号"；但同一行的第 2 格是**故意设置的对照**（同值 + 无样式 ⇒ 按规则**应当**归一），于是报错指名了**本来正确的实现**。
   - **根因**：断言的**被测字段范围**与**语义**不一致 —— 我在一行里放了两格"同值不同样式"，却用"整行"做判据。
   - **防再犯**：**断言先按分隔符切出被测字段**（`md.split('\n')[2].split('|')…`），再判；**对照格给独立期望**、不要与主断言混在一行里判。本次修正后**断言语义未变**（只改被扫范围），已在测试注释里留痕。
3. **native 命令 `2>&1` 又踩一次（本卡是第 3 次）**：`node … 2>&1` ⇒ `StandardErrorEncoding is only supported when standard error is redirected`，**命令根本没执行**（`$LASTEXITCODE` 是残留值，不可采信）。
   - **防再犯**：要合流 stderr 就用 `cmd /c "… 2>&1"`（重定向在 cmd 里做）；**规则早已写进 `AGENTS.md`，栽的是"手滑"而不是"不知道"** ⇒ 别指望记忆，改**习惯**：写完 native 命令**先扫一眼有没有 `|` / `>` / `2>&1`**。
4. **"50 行"这种口口相传的数字，必须回到夹具里数**：卡面把手机侧的观察记成"50 行由 `34.2` 变 `34.200000000000003`"，若照字面理解会去找"整表 50 行"（找不到）。**实测**才发现 = **R 列的 50 个长尾单元格**（整表窗口内其实有 3,039 个）。
   - **防再犯**：**"约 N 行"类描述一律先落成"夹具名 + 列/行范围 + 原始字面值"再动手**（本次正是靠这条把判据钉死，也让"R 列 50 个"成了 A5 差值的解释）。

## 2026-09-20 · 卡 006 回箱核验（Linux 弱独立）：**纪律兑现的一轮**，但我单据内部自相矛盾

> 回箱 `.私档/linux/20260920-alt空白折叠/`（报告 + 回执 + SHA256SUMS + 12 份原始日志 + 5 份自写脚本）。**核验结论：通过** —— 哈希 **25/25 逐条一致**；关键数字全部从**原始日志**复现（组 I2 `7/7`；契约 `290/288/0/2` 与 `287/284/0/3`；两口径对账 EXIT=0；四档 A5 独立台 `66140/23611/352755/352759` 全一致且 `![`×0；metrics 21·525·超限 0·0.4%；guard-selftest 19/19；产物 `134,421 B / 27E78682…A665`；`porcelain = 0`）。

### ✅ 兑现的两条（上两轮的教训真的进了流程）
1. **预跑实测负对照 ⇒ 对方逐项复现一致**：出单前我自己跑了三种注入（A 全绿 / B 只红 I2-4 / A+B 红 5 条 + 组壳）并把**实测**红名单写进单据 —— Linux 侧独立复现**零偏差**。对比卡 005（我写错预期、靠对方纠正）⇒ **"注入点的作用域决定预期"这条纪律已生效**。
2. **blob 级判据被采纳**：本批提交**未推送** ⇒ 无 tree 可对照；按卡 005 的更正，单据给的是**受测文件 blob**，验收方据此核验**四项全中**，并把"无 tree 对照"如实登记为未验证项（没有当成通过）。**⇒ 出单方下单时若提交未推送，就必须给 blob；对方也不许拿"补丁应用成功"当等价证明。**

### 坑（现象 → 根因 → 防再犯）
1. ⭐ **单据内部自相矛盾：§4 改写了，§6 没同步**：
   - **现象**：我在 §4 按**预跑实测**重写了三种注入的预期（注入 B = **只红 I2-4**），但 §6「回箱要求」里**残留第一版草稿**的「注入 B（预期 **5 红** + 组壳）」⇒ 同一份单据里两个互斥口径。Linux 侧只能挑一个执行（挑了 §4），并把矛盾**登记为笔误**。
   - **根因**：改单据时**只改"主要那节"**，没有回扫**引用了同一事实的其他节**（回箱要求、摘要、§5 预期例外都可能复述同一数字）。
   - **防再犯**：**改单据里的任何数字/预期后，立刻 grep 该数字在全文的所有出现处**（本次"5 红"只该出现在"初版错误"的留痕里）；出单前做一次**跨节一致性回扫**（我已有做法：出单前把关键数字列一遍、逐节点名位置）。
2. **目视计数不可靠：清单列了 10 项、标题写「9 文件」**：
   - **现象**：§1「改动面」我手写「9 文件」，而**同一行清单就列了 10 项**，实测（`git diff --stat 437a8c6..HEAD`）也是 10 —— 我脑子里算的是"回执写入前"的 9 文件，清单却是最终态。
   - **根因**：**手写计数**与**清单**取自两个不同时点，且计数没跑命令。
   - **防再犯**：**计数一律贴命令输出**（`git diff --numstat <base>..<head> | wc -l` 之类），不目视数；确需写"约"就写"见命令输出"。
3. **注入态留痕是个好做法（对方的，值得抄）**：他们把三种注入态的产物字节/哈希也记了下来（`134,393 / 134,422 / 134,394 B`）——虽**不作对错判据**，但**证明 `build` 确实随 `src/` 变化**（否则"注入后跑测试"可能跑的还是旧产物）。**⇒ 我方后续出单可把它写成要求。**

## 2026-09-20 · 卡 006（执行线）：图片 alt 的连续空白折叠（用户实测「括号有问题」）

> 完整回执（A1–A11 逐条 · 先红/后绿原始输出 · **A5 前提实测** · 第 7 条对表）见 `docs/任务台账.md` **卡 006** 节；本节只留**一手坑与结论**。

### 交付（未推送 —— 发布动作人执）
- **两处最小改动**（两条入口 = 同一条链）：`src/docx.js` 新增 `docxCollapseWs`（`\s+` → 单空格，**保留既有 trim**）供 `docxAltFromName` 用；`src/html2md.js` `imgFrag` 对 alt 折叠空白（**只折叠、不裁剪首尾** —— 裁剪会动到"无换行输入"的字节）。产物 **134,421 B / `27E78682…A665`**。
- **先红 → 后绿**：契约组 **I2**（7 条，用例页内现造、**既有夹具字节锁零改动**）：先红 `tests 7 / pass 1 / fail 6` → 后绿 `7/7`、全量 **290 / 288 / 0 / 2**。
- **A5「一字不变」**：四档 `Y_TIERS` 实测**全是 `.xlsx`**、产物里 `![` **0 次** ⇒ 本卡改不到它们；离线核对台 **4/4 逐字节** + 浏览器 **Y1 绿**双证。
- **A10 端到端**：等价构造「15 图 + 每张 alt 含空行」⇒ 产物 **15/15 单行**、渲染 **15/15 `<img>`、零裸 `![`**（改前 **0/6**）。

### 坑（现象 → 根因 → 防再犯）
1. ⭐ **量「产物字节」时漏了 `convert()` 的组装步骤 ⇒ 四档恰好各少 113 B，差点误判成"基线漂移/我改坏了"**：
   - **现象**：新写的扫描脚本直接调 `xlsxConvert` 取 `markdown` 与冻结基线比 ⇒ 4/4 全部 `≠基线`，而且**每档都恰好少 113 B**。
   - **根因**：`convert()` 在 `truncated` 时会在产物**首行**插截断注释（`withTruncationNotice`，`src/convert.js`）⇒ 直接调转换器拿到的 `markdown` **不是产物**。113 B 正是那条注释的长度（四档都被截断 ⇒ 同一行）。
   - **防再犯**：① 判「产物字节」必须走**与产物同口径**的组装（卡 005 的 `.tmp/card005/y1-offline.mjs` 里已有 `assemble()`，本次另写脚本时**没复用** ⇒ 写这类台子先找现成的）；② **"所有档位差同一个常数"先怀疑口径，别先怀疑自己改坏了**（常数差 = 系统性差异；真回归不会这么整齐）。
2. ⭐ **XML 属性值里的"字面换行"会被归一化成空格 ⇒ 夹具写不对就造不出红回路**：
   - **现象**：按 Word 真实形态造 `descr` 时，若在 XML 属性里写**字面换行**，DOM 解析回来是**空格** ⇒ `![alt](src)` 根本不跨行 ⇒ **红回路建不起来**（也就测不到本卡的病）。
   - **根因**：XML 规范对**属性值**做 normalization（字面空白 → 空格），但**字符引用（`&#10;` / `&#9;` / `&#13;`）不参与归一化** —— Word 存多行「可选文字」正是用 `&#10;`。
   - **防再犯**：造 XML 夹具时**属性里的空白一律写字符引用**；并且断言里把 `\n` **显形**（本次用 `JSON.stringify(md)` 打出来核对）。
3. **卡面 non-goal 的「前提」也可能是错的 —— 实测优先于卡面陈述**：
   - **现象**：卡面写「`html2md` 有 `%5D` 转义、**docx 路径没有**」⇒ 要我把「docx alt 的 `]` 未转义」登记为另一张卡的候选。**实测**：`descr="A]B](https://example.invalid)&#10;C"` ⇒ 产物 `![A%5DB%5D(https://example.invalid) C](assets/alt-bracket-1.png)` —— **`]` 已被 `%5D` 转义**。
   - **根因**：docx 的 alt **不直接拼 markdown**，而是 mammoth → HTML `<img alt>` → `htmlToMarkdown` → `imgFrag`（转义在那里）⇒ 卡面把「两条入口」当成了并列关系，实际是**一条链**。
   - **防再犯**：写「某路径没有 X」之前**跟数据流走一遍**；登记候选之前先做一次最小实测（本次 1 分钟就推翻了前提；不测的话下游会照着一张错卡开工）。

## 2026-09-20 · 卡 005 回箱核验（Linux 弱独立）：材料可复核，**但暴露我单据的两处缺陷**

> 回箱：`.私档/linux/20260920-库回退降级可见性/`（报告 20,939 B + 回执 + `SHA256SUMS.txt` + `summary.json` + 18 份原始日志 + 5 份自写脚本）。**核验结论：通过** —— 哈希清单 **24/24 逐条一致**；关键数字我**全部从原始日志复现**（G8 `7/7`、带夹具 `283/281/0/2`、干净检出 `280/277/0/3`、两口径对账 EXIT=0、四档 `66140/23611/352755/352759`、metrics 524/超限 0/0.4%、guard-selftest 19/19、产物 `134,317 B / 23CEC085…4FC7`）；负对照**真红**后逐字节还原。

### 坑（现象 → 根因 → 防再犯）
1. ⭐ **出单方（我）写的「负对照预期红名单」是错的 —— 因为注入点的作用域比被测断言大**：
   - **现象**：我写「§4 方案 A（`xlsxDegradeWarnings` 首行 `return [];`）⇒ G8-1/G8-3/G8-4 红，G8-2/G8-5/G8-6 绿（3 红）」；Linux 侧实测 **4 红**（`# tests 7 / pass 2 / fail 5`）—— **G8-2 也红**。
   - **根因**：`xlsxDegradeWarnings()` 是**三类降级告警（表宽 / 环境 / 日期）的唯一出口**。整体短路 ⇒ 连 G8-2 **正向断言**的那条「cellXfs/日期」告警也一起关掉，而 G8-2 的后半句正是断言"日期提示**须照报**"。（G8-2 的前半句"dimension 正确时不得报表宽"仍绿 —— 所以它是**一条混装断言**。）
   - **防再犯（出单侧）**：负对照的注入点要写成**代码级锚点**（函数 + 行 + 具体语句），并按**注入的实际作用域**推导预期红名单 —— **若注入点是「唯一出口」型函数，预期必须覆盖它的全部下游正向断言**；最好同时给**窄注入**版本（本次 Linux 侧补的 A′「只短路表宽分支」⇒ 恰好 3 红，正是我本想的形状）。
   - **防再犯（实现侧）**：混装断言（一半负对照 + 一半正向）在**归因**上更贵 —— 拆成两条更利于定位。本次不改断言（断言即规格，且它工作正常），仅登记。
2. **amend 之后，单据里写死的提交哈希当场作废**：交接单写实现提交 `28e3618`，而那是 **amend 前**的哈希（补丁也从它生成）；远端实际是 `0d4f039`。⇒ Linux 侧 `git am` 后 `HEAD^{tree} ≠ 0d4f039^{tree}`（差 `docs/任务台账.md` 4 行自指说明）。
   - **防再犯**：出单**同时贴**「补丁 `From` 哈希」+「远端提交哈希」并注明是否 amend；**判据写成受测文件 blob 级相同**（`git rev-parse :<path>`）而非整树相同 —— 本次正是 blob 级比对证明"两个态测的确实是同一份代码"（`src/xlsx.js` `df5b452e…` / `index.html` `213596fa…` / `tests/contract_v1.test.mjs` `6eac2109…` 三份全同）。
3. **单据里的状态行（"未推送"）是写单时刻的快照**：本次写单与用户推送**并行发生** ⇒ 交付时已过期。
   - **防再犯**：单据状态性内容一律标「**以现跑为准**」，回箱核验时**现场回读**（`git rev-parse origin/main` + `gh run list`）。
4. **回箱核验必须复核原始数据，不能读结论**（本次复核收益 = 抓到上面两条）：做法 = ① 逐条重算 `SHA256SUMS.txt`；② 关键数字**只从 `原始日志/` 里取**（TAP 摘要行、EXIT 行）；③ 与自己的本机/CI 值做**三方比对**；④ 负对照要看**具体哪几条红 + 原始报错**，不是"红了就行"。
...
- **等维护窗口结束后重跑失败作业**：`gh -R sakuraqqq/doc2md run rerun 35458379084 --failed`（由执行线代跑；**这是唯一能拿到卡 005 真 CI 结果的路径** —— 首次 run 的测试步从未执行）。✅ **已完成**：端点 **17:39:46Z 即恢复 200**（比官方窗口 19:00 UTC 提前）⇒ 先在本机复跑该步 **PASS** 才动手 ⇒ 重跑 **`✓ checks in 1m20s`，16 步全 ✓**，其中 ⭐ **`Contract ↔ BASELINE.json reconciliation` ✓** —— 即真实 runner 的 TAP 与人工同步的 `ci_clean_checkout = 280/277/0/3` **逐项一致**（与"我本机 + Linux 弱独立"构成**三方吻合**）。
- **加固建议**：写进 `HANDOFF §8` 第 6 条（**待立卡材料**）；`tools/**` 与 `.github/**` 都在卡 005 范围外 ⇒ **本卡不动**。
- **⚠️ 更正（同日，本段原写法作废）**：本节初版写「本回执（`docs/任务台账.md` §005）不改」—— 随后**用户拍板落盘**且 **Linux 回箱到达**，回箱核验与 CI 终态需要与回执同处留痕 ⇒ 现已在 §005 追加「**回箱核验 + CI 终态**」小节（含我单据两处缺陷的更正）。原写法作废，理由如上（更正留痕，不静默删）。
...
### 交付（未推送 —— 发布动作人执）
- **只动 `src/xlsx.js`**（+199/−12）：回退分支现在带**降级观测** —— 读工作表头部 `<dimension>`（声明的列/行）vs 扫前 1000 行取**实际**列/行，只有**真的少**才报（文案含「右侧 N 列已被丢弃」）；**环境线**（无 `DecompressionStream`）给性质提示；**日期风险**（缺 `<cellXfs>`）按口径②**只加 warning、不改回退逻辑**。**007 修复**：回退分支不再丢 sheet 名（`[null]` → 宽松解析取回真名，并按名读取）。
- **`src/convert.js` / `src/ui.js` 一行未动**：`res.warnings → meta.warnings` 汇总与 `.warnings` 渲染**早已具备**（卡面写的是「若需」）⇒ 提示自动进 UI（真 Chrome 实测：结果卡内 `⚠ …` 一条，两段提示以「；」相连）。
- **先红 → 后绿**：契约组 **G8**（7 条；用例页内 `fflate` 现造、不入库样例）—— **先红** `tests 7 / pass 2 / fail 5`（G8-1/2/3/4 红）→ **后绿** 全量 **283 / 281 / 0 / 2**、G8 **6/6**。
- **产物零变化**：四档 md 字节 + SHA256 与冻结基线**逐字节一致**（本批只改「提示」与「sheet 名」，不碰 markdown）。

### 坑（现象 → 根因 → 防再犯）
1. ⭐ **为过 `metrics` 拆函数，把「循环 `continue` 的二次检查机会」吃掉了 ⇒ 新功能静默失效，而三道检查全绿**：
   - **现象**：`probeSheetArea` 首版把 `if (!dimDone) dimDone = takeDimension(p.pending, probe) || …` 放在循环头 —— 因为「取不到完整 `<row>` 就 `continue`」会**回到循环头再检查一次**，首次 `pullRowUnit` 拉回数据后的那次检查才看得见 `<dimension>`。`metrics` 报 `OVER src/xlsx.js:751 probeSheetArea cyc=9 cog=16` 后，我把「拉行单元」抽成 `pullRowUnit`（内含 `while (true)`）⇒ 控制流**不再回到循环头**，`<dimension>` 在第一次「取到行 ⇒ 消费前缀」时被一并吃掉 ⇒ `probe.declared` 恒为 `null` ⇒ **表宽告警永不出现**。
   - **为什么三道存量检查都没抓到**：① `npm run lint` **0 error / 0 warning**（只看结构）② `npm run metrics` **超限 0**（它只数复杂度，不数语义）③ **离线核对台 4/4 绿** —— 四档大夹具**都没有 `<dimension>` 元素**（已实测：`tests/data/*.xlsx` 10 件同样全无该元素）⇒ 该分支**根本没被覆盖**。**唯一变红的是先红断言 G8-1/G8-3**。
   - **根因一句话**：拆除控制流结构时，**隐式依赖「循环头会被再次执行」的副作用**没有被识别成依赖。
   - **防再犯**：① 为过门禁而拆函数后，**必须重跑该功能的先红断言**（别只看 lint/metrics）；② 观测类逻辑写成**显式阶段**（阶段①读声明、阶段②扫行，互不消费缓冲），语义不依赖控制流细节 —— 本次最终实现即如此；③ **夹具要同时覆盖「有/无」两种形态**（既有夹具全属「无 `<dimension>`」⇒ 等于零覆盖，这正是坑能溜过去的原因）。
2. **`metrics` 的认知复杂度会惩罚「循环 + 提前 `continue`」这种自然写法**：本例 `cyc=9 cog=16` 被判超限。⇒ 正确的降复杂度方向是**抽出一个语义完整的子步骤**（本例 `pullRowUnit`），**不是**把判定拆成多个小 `if`——后者恰好制造了上面的静默失效。
3. **「不加断言的新功能」等于没有验收面**：本批新增的三类提示（表宽/环境/日期）**全都是 `meta.warnings` 里的字符串**，没有断言时它们「有或没有」都不影响任何门禁。⇒ 新功能**先写能变红的断言**（G8 的四条红），是本批唯一抓住坑 1 的手段。
...
### 交付（未推送 —— 发布动作人执）
- `tools/baseline-check.mjs` 新增 `--from-tap <tap> --scope <口径>`：解析 TAP 摘要 ↔ `docs/BASELINE.json` 的 `contract[scope].counts` **逐项比对**，不符 **exit 1**，**绝不自动改写 JSON**；`tools/guard-selftest.mjs` **16 → 19/19**（2 负例 + 1 正例）；CI 在 `npm test` **之后**加对账步（显式 TAP reporter，`tap.txt` 落地）。
- **本机两口径对账双双 PASS**：`local_with_fixtures` `276/274/0/2`、`ci_clean_checkout` `273/270/0/3`（后者用"夹具指向仓库内不存在目录"复现 CI 的干净检出）—— **未改任何应然值**。

### 坑（现象 → 根因 → 防再犯）
1. ⭐ **默认 reporter 随 Node 版本变（Node 20 = `tap` / Node 24 = `spec`）**：本机把 stdout **重定向到文件（非 TTY）**跑 `node --test`，输出的是 **spec**（`ℹ tests 1` / `✔ tests\…`）；而 CI（Node 20）日志里是 **tap**（`# tests 273`）。⇒ 若解析端依赖默认值，**本地与 CI 会行为不一致**，而且失败是**静默**的（解析不到就当没数据）。
   - **防再犯**：**消费机器输出就必须显式指定格式**（`--test-reporter=tap`），并对"格式不符"**明确报错**（`--from-tap` 对非 TAP 输入 exit 1 并提示该怎么跑）—— **"解析不到"绝不能静默当 0**。
2. ⭐ **`npm test | tee` 把退出码吞了**：管道的退出码 = `tee` 的（0）⇒ **测试挂了那一步照样绿**。CI 里这是最危险的一类"假绿"。
   - **防再犯**：`set -o pipefail`（已写进 workflow 注释，标明"不能省"，防后人删）；本地流程同理（要么不接管道，要么显式取 `PIPESTATUS[0]`）。
3. **lint 退出码 0 也可能带 warning，而 metrics 会把它当超限**：新增的 `reconcileTap` 首版**圈复杂度 12** ⇒ `npm run lint` 只给 **warning 且 exit 0**，但 `npm run metrics` 的硬门禁会把它算成**超限函数**（CI 红）。⇒ 拆函数后 **0 warning**。
   - **防再犯**：改 `tools/**` 后**必须同时看 lint 输出文本与 `npm run metrics`**（只看退出码会漏）；建议把 `npm run lint` 改成 `--max-warnings=0`（`package.json` 不在卡 003 范围，列为建议）。
   - 备注：这是本项目**第二次**栽在"两套复杂度实现"上（卡 001 是检查脚本被 metrics 判超限）⇒ 已够"同类 ≥2 次"，可作沉淀候选。

## 2026-09-19 · 卡 002（执行线）：大文件流式化 + 撤护栏 + A9/A10 预检提示

> 完整回执（A1–A14 逐条验收 · 第 7 条对表 · 字节+SHA 证据 · 残留）见 `docs/任务台账.md`；本节只留**一手坑与结论**。

### 交付（未推送 —— 发布动作人执）
- 两提交、顺序写死（**先流式化 → 再撤护栏**；中间态未上线）：`3fc54a9`（`src/xlsx.js` 三处流式化 + `meta.scan`）→ `4da6ac4`（撤 4 MB `sharedStrings` 护栏 + A9 预检 + A10 提示/双让帧）。
- `npm test`（本机 · 升权 · **有夹具**）：**276 tests / 274 pass / 0 fail / 2 skip / 56.1 s**；组 Y 三绿 —— `3-big` **96,793 ms → 367 ms**、`4-mid-large` **65,824 ms → 301 ms**；`meta.scan` 两档 **802,816 B**（≤ 8 MB、**差 0%**）。
- **Y1 冲突处置（本批最大发现）**：大档**原冻结基线 = 库回退路径产物**（护栏按**压缩**体积触发）⇒ 撤护栏后大档改走快路径，而两路径对这些文件**本来不等价**（表宽 18 → 22 列）⇒ 用户拍板**重冻大档基线**；**等价性台**证明**流式化本身零字节变化**（旧快路径（仅关护栏）vs 新流式 ⇒ 四档 markdown **逐字节相同**）。

### 坑（现象 → 根因 → 防再犯）
1. ⭐ **esbuild 打包序把顶层状态擦成 `null`（最隐蔽：功能静默失效、零报错）**：`src/convert.js` 用 `let preflightHook = null` 存 UI 注册的钩子，`src/ui.js` 顶层 `setPreflightHook(...)` 注册；打包（`format=iife`、各模块展平进**同一个 IIFE 作用域**）后 **convert.js 的顶层语句排在 ui.js 之后执行** ⇒ 注册被 `= null` **覆盖** ⇒ 提示永不出现，**控制台零报错、测试全绿**。
   - **根因**：跨模块共享状态**放在模块顶层 `let/var` 初始化**时，其赋值时机取决于打包器的语句排布，**不保证在"使用方注册"之前**。
   - **怎么发现的**：浏览器里探针观察 `#status` 的 `textContent` 写入 —— **一次写入都没有**；再 `node -e` 检查产物：`xlsxPreflight`/`setPreflightHook`/`zipDirectory` 都在包里、`zipDirectory` 在 Node 里对同一字节算得出 361 MB ⇒ 排除"没打包"和"算不出"，只剩钩子被擦。
   - **防再犯**：跨模块共享状态**不要**依赖顶层初始化顺序 —— 改成「挂在（会被提升的）函数对象上的**懒槽**」（`preflightHooks().fn`，首次访问创建）；**新增"UI 钩子"类接线后必须真跑一次浏览器实测**（静态检查、单测、lint 都看不出来）。
2. **冻结基线可能锚在"另一条实现路径"上**：只改「走哪条路径」的批次，**动手前先确认冻结值由哪条路径产出**。本批 `src/xlsx.js` 的护栏按**压缩**体积（`compSize`）判定 > 4 MB ⇒ 两档大夹具（16.4 / 10.4 MB）**从来走的是库回退路径**，Y1 冻结的就是库路径产物；撤护栏后必然改走快路径 ⇒ 冻结值与新路径天然不同。
   - **防再犯**：① 判据要按**真正相关的量**（这里是"解压后规模"）而非代理量（压缩体积）；② 冻结"产物不变"类基线时，**记录产出它的实现路径**（本批已写进 `tests/CONTRACT.md` §7 与 Y1 注释）；③ 用**等价性台**（旧路径 vs 新路径，同一实现内）把「改读法」与「改路径」两件事分开证。
3. **沙箱口径（第三次撞，与既有登记一致）**：`npm run build`（esbuild 服务子进程）与 `npm test`（test runner）**都要一次性升权**；`node tools/*.mjs`、`node --input-type=module -e` 之类的纯脚本可在沙箱内跑 —— 本批的 **Y1 离线核对台**（不上浏览器，直接调 `xlsxConvert` 比对冻结字节+SHA）就是靠这条在沙箱里迭代的。
4. **出单方写的"环境变量语义"没按运行链路核对（Linux 侧实测打回）**：我在交接单里写「夹具可复制到**仓库外**目录，用 `DOC2MD_BIG_FIXTURES` 指过去」——**实测会让组 Y 变红**：契约组 Y 取夹具走**同源 HTTP fetch**，`yFixtureUrl()` 用 `path.relative(ROOT, Y_FIXTURES)` ⇒ 仓库外目录算出的 URL 带 `..`，服务端按站点根解析取不到 ⇒ 四档全取不到 ⇒ Y 红。**正确做法 = 软链到仓库内**（Linux 侧即用此法通过）。
   - **根因**：该环境变量在实现里**只对"仓库内可达路径"有意义**（浏览器取数受站点根约束），我在出单时只核了"文件存在"，没核"运行链路能不能读到"。
   - **防再犯**：出单写明某个开关/变量时，**必须沿真实取数链路走一遍**（谁读、从哪读、有没有根约束）；**"文件存在"≠"程序读得到"**。
5. **同一现象、三种内存口径（判据必须先写死窗口）**：Linux 侧一次运行给出三个都"正确"的数：**保留量 +0.2 MB**（修前 +6.4–7.4 GB，强证脱钩）· **窄窗口**（convert 调用内）**+468.3 MB**（达标）· **宽窗口**（含 47.4 MB 文件装载）**+646.0 MB**（超 500 MB 阈值）。
   - **根因**：判据只写了"内存 < X MB"，**没写窗口定义与指标口径**（保留量 / 边际 / 进程树 RSS 峰值；PSS vs RSS；是否含装载）。
   - **防再犯**：性能/资源类判据一律**三件套齐**：**指标 + 窗口 + 采样口径**（本批已在回执与 `CONTRACT` §7 登记待拍板：建议统一到"转换边际（基线→峰）"）。
