...
3. **全局 `~/.dsh/AGENTS.md`：改不了 —— 如实上报**（工作区外 + 沙箱拒绝写入 + 一次性升权被用户拒绝 ⇒ **不绕行、不找旁路**；按协作铁律 2/4）。
...
3. **"用户说全局也改" ≠ "全局真的有内容可改"**：核实结果与用户预期不符时，**直接说"没有"并给证据**，不要为了顺着预期去"先造一处、再改掉它"。
4. **同名规则的第二个存放点：跨会话记忆库**：`mneme` 里存着 `[constraint] 重构配额（童子军规则）：每次改动顺手做小重构，≤50 行…`（**每次会话自动注入**，效果上"最像全局配置"的东西其实是它）。⇒ 规则改写后**记忆条目会与权威源打架**，已列为**待用户拍板项**（改 / 删由用户定，按记忆纪律"先提议后写入"）。
